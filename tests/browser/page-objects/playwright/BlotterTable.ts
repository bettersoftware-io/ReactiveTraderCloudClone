...
export class PlaywrightBlotterTable implements BlotterTablePO {
  ...
  async firstRowBackgroundColor(): Promise<string> {
    return await this.firstRow().evaluate((el) => {
  ...
  async expectContainsText(text: string, timeoutMs: number): Promise<void> {
    await expect(this.locator()).toContainText(text, { timeout: timeoutMs });
  }
}
