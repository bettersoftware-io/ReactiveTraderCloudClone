...
Then("the view toggle button is visible", function (this: StepContext) {
...
When("the trader clicks the view toggle", function (this: StepContext) {
  return fxLiveRates.clickViewToggle(this.ctx);
});

When("the trader records the first tile text", function (this: StepContext) {
  return fxLiveRates.recordFirstTileText(this.ctx);
});

When("the trader waits {int} seconds", function (this: StepContext, n: number) {
  return common.waitSeconds(this.ctx, n);
});

Then("the first tile text is non-empty", function (this: StepContext) {
