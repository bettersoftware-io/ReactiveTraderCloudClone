...
export function toggleAndCaptureBackgrounds(ctx: TestContext): void {
  chainable(ctx.po.workspace.rootBackgroundColor()).then((c) => {
    ctx.scratch.theme.backgroundBefore = c;
  });
  void ctx.po.themeToggle.click();
  chainable(ctx.po.workspace.rootBackgroundColor()).then((c) => {
    ctx.scratch.theme.backgroundAfter = c;
  });
}
...
export function expectBackgroundMatchesToggled(ctx: TestContext): void {
  chainable(ctx.po.workspace.rootBackgroundColor()).then((current) => {
    assertEquals(
      current,
      ctx.scratch.theme.backgroundAfter,
...
export function expectThemeToggleAriaLabelMentions(
  ctx: TestContext,
  term: string,
): void {
