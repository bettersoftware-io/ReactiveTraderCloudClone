...
export function recordBlotterRowCount(ctx: TestContext, key: string): void {
  chainable(ctx.po.blotterTable.rowCount()).then((n) => {
    ctx.scratch.blotter.recordedRowCounts.set(key, n);
  });
}
...
export function expectBlotterRowCountAtMost(
  ctx: TestContext,
  key: string,
): void {
...
export function expectBlotterRowCountEquals(
  ctx: TestContext,
  key: string,
): void {
  chainable(ctx.po.blotterTable.rowCount()).then((count) => {
