...
When("the browser goes offline", function (this: PresenterWorld) {
  return conn.browserGoesOffline(this);
});

When("the browser comes back online", function (this: PresenterWorld) {
  return conn.browserComesBackOnline(this);
});

When("the gateway connection drops", function (this: PresenterWorld) {
  return conn.gatewayDrops(this);
});

When("the gateway attempts to reconnect", function (this: PresenterWorld) {
  return conn.gatewayAttemptsReconnect(this);
});

When("the gateway connection is restored", function (this: PresenterWorld) {
...
Then("the connection overlay is hidden", function (this: PresenterWorld) {
