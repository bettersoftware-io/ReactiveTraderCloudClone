// tests/scenarios/cypress/fxLiveRates.ts
// Cypress fork of tests/scenarios/fxLiveRates.ts — synchronous bodies, queue-aware.
...
export function expectFirstPriceTileVisibleWithin(ctx: TestContext, seconds: number): void {
...
export function expectFirstTileHasBuyAndSellButtons(ctx: TestContext): void {
  chainable(ctx.po.liveRatesTile.firstTileSellVisible())
    .then((v) => assertTrue(v, "first tile sell button not visible"));
  chainable(ctx.po.liveRatesTile.firstTileBuyVisible())
    .then((v) => assertTrue(v, "first tile buy button not visible"));
}

export function recordVisibleTileCount(ctx: TestContext, key: string): void {
  chainable(ctx.po.liveRatesTile.count())
    .then((n) => { ctx.scratch.fxLiveRates.recordedCounts.set(key, n); });
}
...
export function expectVisibleTileCountAtMost(ctx: TestContext, key: string): void {
...
export function expectVisibleTileCountEquals(ctx: TestContext, key: string): void {
  chainable(ctx.po.liveRatesTile.count())
    .then((n) => {
      const baseline = ctx.scratch.fxLiveRates.recordedCounts.get(key);
      if (baseline === undefined) throw new Error(`no recorded count for ${key}`);
...
export function expectViewToggleVisible(ctx: TestContext): void {
  chainable(ctx.po.liveRatesTile.viewToggleVisible())
    .then((v) => assertTrue(v, "view toggle not visible"));
}

export function expectViewToggleShows(ctx: TestContext, expected: string): void {
  chainable(ctx.po.liveRatesTile.viewToggleLabel())
    .then((label) => assertContains(label, expected));
}
