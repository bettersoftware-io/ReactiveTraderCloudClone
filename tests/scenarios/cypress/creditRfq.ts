// tests/scenarios/cypress/creditRfq.ts
// Cypress fork of tests/scenarios/creditRfq.ts — synchronous bodies, queue-aware.
...
export function expectCreditTabVisible(ctx: TestContext, tab: string): void {
  ensureCreditTab(tab);
  chainable(ctx.po.creditRfqPanel.tabIsVisible(tab))
    .then((v) => assertTrue(v, `credit tab not visible: ${tab}`));
}
...
export function expectCreditRfqHasBuySellButtons(ctx: TestContext): void {
  chainable(ctx.po.creditRfqForm.hasBuyAndSellButtons())
    .then((v) => assertTrue(v, "credit RFQ form missing Buy/Sell buttons"));
}

export function expectCreditRfqHasDirectionLabel(ctx: TestContext): void {
  chainable(ctx.po.creditRfqForm.hasDirectionLabel())
    .then((v) => assertTrue(v, "credit RFQ form missing Direction label"));
}
