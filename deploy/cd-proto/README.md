...
HTML under `docs/design/<version>/standalone/`) to its own Vercel project,
`rtc-clone-cd-proto` → **https://rtc-clone-cd-proto.vercel.app**, behind a shared
password.

This is the *design mockup*. The readable **React port** (`@rtc/client-prototype`)
...
3. Copies the chosen HTML in as `index.html` (never committed — it's an 835 KB
   duplicate).
4. `vercel deploy --prod` to the `rtc-clone-cd-proto` project, then smoke-tests
   that an unauthenticated request is gated (`401`).
...
`SITE_PASSWORD` is an environment variable on the `rtc-clone-cd-proto` Vercel
