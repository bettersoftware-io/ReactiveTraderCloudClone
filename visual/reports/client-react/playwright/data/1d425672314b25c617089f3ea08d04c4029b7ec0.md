...
- Name: visual.spec.ts >> tile/rfq-received__classic-dark
...
Locator: getByTestId('scenario-root')
  ...
  Snapshot: classic-dark/tile-rfq-received.png

Call log:
  - Expect "toHaveScreenshot(classic-dark/tile-rfq-received.png)" getByTestId('scenario-root') with timeout 5000ms
...
```ts
  ...
  50 |     // engine-deterministic, so any diff at all is a real divergence. A
  51 |     // scenario-level maxDiffPixels instead RAISES the absolute cap for a
  52 |     // measured, mechanism-understood divergence (see Scenario's doc).
  53 |     const strictOpts = scenario.strict
  54 |       ? { maxDiffPixels: 0, maxDiffPixelRatio: 0 }
  55 |       : scenario.maxDiffPixels !== undefined
  56 |         ? { maxDiffPixels: scenario.maxDiffPixels }
  57 |         : {};
  58 | 
  59 |     if (action.fullPage) {
  60 |       await expect(page).toHaveScreenshot(shot, {
  61 |         animations: "disabled",
  62 |         fullPage: true,
  63 |         ...strictOpts,
  64 |       });
  65 |     } else {
> 66 |       await expect(page.getByTestId("scenario-root")).toHaveScreenshot(shot, {
     |                                                       ^ Error: expect(locator).toHaveScreenshot(expected) failed
