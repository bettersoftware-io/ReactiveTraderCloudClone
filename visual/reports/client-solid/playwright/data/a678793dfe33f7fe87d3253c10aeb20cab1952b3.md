...
- Name: visual.spec.ts >> connection-overlay/offline__classic-dark
...
Error: expect(page).toHaveScreenshot(expected) failed
  ...
  2006538 pixels (ratio 0.97 of all image pixels) are different.

  Snapshot: classic-dark/connection-overlay-offline.png

Call log:
  - Expect "toHaveScreenshot(classic-dark/connection-overlay-offline.png)" with timeout 5000ms
...
```yaml
- paragraph [ref=e6]: This device has been detected to be offline. Connection to the server will resume when a stable internet connection is established.
```
