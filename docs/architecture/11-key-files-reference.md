...
| **Visual Golden Tiers** | `packages/client-react/tests/ui/visual/{playwright-ct,playwright,vitest-browser}/` (runners/config) + `packages/ui-contract/goldens/{playwright-ct,playwright,vitest-browser}/__screenshots__/` (the goldens themselves) | Three screenshot runners; dual golden sets (`react/` CI-canonical + `react-local/<arch>/`), generated only from `client-react`; ADR-001 lives with the runners |
