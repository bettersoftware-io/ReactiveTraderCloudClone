...
The same tree once more, drawn the way [Thinking in React](https://react.dev/learn/thinking-in-react) draws a mock-up — as boxes nested to the screen geometry the FX tab actually renders (same color code as above). Every label is a component name exactly as React DevTools shows it, and the green boxes double as a picture of the layout data: each one renders one `kind: "split"` node of `FX_ROOT`, drawn to its `sizes` ratios:

![The FX tab as nested component boxes: App wraps AmbientBackground, HeaderChrome, WorkspaceEngine, StatusBar and the overlay sheets; inside WorkspaceEngine, the two view providers wrap InhouseLayoutEngine, whose SplitNode boxes hold the four FX panels resolved from the registries](17-fx-devtools-boxes.svg)

Two DevTools footnotes keep the picture honest. The amber panel sections, the cells that size them, and the resize handles between them are host elements (`section.panel`, `div.cell`, `hr.handle`) emitted by `renderPanel` — a plain helper function, not a component (`InhouseLayoutEngine.tsx:515`) — so the components panel shows each head component and its `PanelErrorBoundary ▸ <panel>` pair sitting directly under a `SplitNode` (the one construct here that *is* a real component, extracted so its drag `useRef` obeys the rules of hooks, `InhouseLayoutEngine.tsx:253-257`), with no wrapper components in between. And the two dashed sheets, `ConnectionOverlay` and `LockScreen`, are permanent leaves of `App` in DevTools but usually absent from the screen — each renders `null` until its presenter seam activates it (`ConnectionOverlay.tsx:15-17`, §17.5).
