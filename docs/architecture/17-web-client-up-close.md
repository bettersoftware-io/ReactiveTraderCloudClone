...
A sibling rule, `rtc/name-functions-by-effect` (`eslint-rules/name-functions-by-effect.mjs`), holds every plain function to the standard `rtc/no-render-functions` above holds components to: the name must state what actually happens, never the occasion that triggers it. `handleClick`, `onMessage`, `processClick`, and `frameCallback` (when the binding holds a function) all fail — an effect verb plus a domain noun is the bar, because a name like that survives being rewired to a different trigger (`handleClick` → `dismissTicket` still reads right if the trigger later moves from a click to a keypress; `handleClick` → `handleClicked` does not). The rule turns on one distinction, the same one `<Car onYellowLight={prepare} onRedLight={stop} />` illustrates in its own doc comment: a SLOT is a placeholder a consumer attaches a handler to — its declarer cannot know what the consumer will do and must not try to name it, so `onYellowLight` is correct as written, and `onX` **props** are exactly this shape and are left untouched. A CONCRETE HANDLER (`prepare`, `stop`) receives the event, or nothing, and *is* what runs, so it has to be named for its effect. Because "who invokes this" is invisible to a bare declaration, the rule discriminates a slot from a handler purely by syntax, and that produces the one non-obvious consequence worth knowing before writing a prop: a function-typed member declared in **property** syntax (`onExecute: (d: Direction) => void`) is a slot and exempt, but the identical intent declared in **method** syntax (`onExecute(d: Direction): void`) parses as a command whose sole parameter is data rather than a callback, and it IS flagged. A genuine prop slot therefore has to be written as a property, never a method — this was not a pre-existing convention in this codebase; the migration that shipped this rule converted 67 members to property syntax to establish it (6 in `client-react`, 6 in `client-solid`, 55 in `client-prototype`). Two limits are worth carrying forward before trusting a green lint here. First, the rule cannot verify the *replacement* — it only makes the lazy name unavailable, so a passing run is not an endorsement of whatever name replaced it. Second, `handle` is also a legitimate domain noun in this shell (the resize handles between split panes, `§17.1` above), so `LayoutEnginePage.handleExists` (asking whether a resize handle exists) was a true false positive; the fix was the same disambiguating rename the rule always calls for — `resizeHandleExists` — never a disable. Design: [`docs/superpowers/specs/2026-07-26-name-functions-by-effect-design.md`](../superpowers/specs/2026-07-26-name-functions-by-effect-design.md).
...
**2. `useFlipGrid` — FLIP over raw WAAPI for keyed-grid reorders.** `useFlipGrid` (`packages/client-react/src/ui/shell/motion/useFlipGrid.ts:29-154`) implements FLIP (First-Last-Invert-Play). Consumers pass a `register(key)` ref callback to each grid item (`useFlipGrid.ts:135-151`); whenever the caller's `deps` array changes, a `useLayoutEffect` (`:79-133`) re-measures every registered element's `getBoundingClientRect()` — First, captured as `prevPositions` from the previous pass — lets React's own re-render already reorder the DOM — Last — computes the inverse delta between old and new rects (`flipDeltas`, extracted to `@rtc/motion-core` at `packages/motion-core/src/flip.ts:33-55` and called at `useFlipGrid.ts:84`) — Invert — and plays it as a raw `Element.animate` translate-to-none tween (`playFlip`, `:188-193`, `440`ms `cubic-bezier(.22,.85,.3,1)` — the `FLIP_DURATION_MS`/`FLIP_EASING` constants also live in `@rtc/motion-core` at `flip.ts:16-17`) — Play. Two production consumers: the Live Rates grid FLIPs on currency-category filter changes (`useFlipGrid([filter], { enter: true, exit: true })`, `LiveRatesPanel.tsx:34`, tiles registered at `:52`), and the credit RfqsPanel FLIPs on filter-or-visible-id-set changes (`useFlipGrid([filter, renderedIdsKey])`, `RfqsPanel.tsx:194`).

Two subtleties the hook's own comments call out. First, a separate effect (`:48-77`) keeps a `ResizeObserver` plus a `window` `resize` listener that re-measure and overwrite the stored origins WITHOUT animating whenever the grid's geometry changes for a reason other than a tracked dep — a browser resize, or a dock panel being dragged/resized (both change tile sizes) — so the next real deps-change FLIP starts from the current layout, not a stale one (`useFlipGrid.ts:43-47`). Second, that refresh is itself guarded: `getBoundingClientRect` includes in-flight WAAPI transforms, so a refresh landing mid-glide would capture a transformed rect and store it as the next FLIP's origin — reading as a jump-cut. `anyGlideRunning` (`:161-169`) skips the refresh whenever any registered element still has a running `Animation`; a skipped refresh self-heals because the deps-change effect re-measures from scratch regardless the next time `deps` changes (`:48-77,79-133`).

The hook's own doc comment gives the reason it reaches for raw WAAPI here instead of the `animateOnce` Motion One wrapper below: "FLIP fires per-item on every layout, so a short-lived native animation avoids spinning up the Motion One engine per grid item" (`useFlipGrid.ts:20-24`).

`useRankGlide` (`packages/client-react/src/ui/equities/watchlist/useRankGlide.ts:120-244`) is the 1-D sibling for the equities watchlist's rank column. Instead of measuring DOM rects it derives a `translateY` delta from each symbol's index move in the sort order (`computeRankDirections`, extracted to `@rtc/motion-core` at `packages/motion-core/src/rankGlide.ts:11-28`) and layers in a direction-colored highlight pulse — green on a rank rise, red on a fall (`playHighlight`, `:67-100`). Its header comment records a gate `useFlipGrid` doesn't need: the default "chg" sort re-derives the order from up to six independent 500ms quote streams — as many as ~12 candidate reorders/sec against a 560ms glide (`:12-27`) — so committing every candidate would start overlapping animations and corrupt the mid-glide row-height measurement (rows visibly stacking/overlapping). `coalesceOrder` (in `@rtc/motion-core` at `rankGlide.ts:63-82`, applied at `useRankGlide.ts:143`) buffers all but the latest candidate while a glide is in flight and applies it only once the current glide's WAAPI `.finished` promises settle (`:224`).

**3. `animateOnce` — the Motion One wrapper, staged for one-shot entrances.** `animateOnce` (`packages/client-react/src/ui/shell/motion/index.ts:10-16`) wraps `motion`'s `animate(...).finished` in a promise the caller can `await`. Its module doc comment frames it as "the single Motion One import site. Everything else in the UI animates through this wrapper so the engine stays swappable" (`index.ts:3-7`) — a Solid client would swap this one file for `solid-motionone` without touching any consumer, since nothing outside it imports Motion One directly. This is one prepared seam the shipped SolidJS port never had to exercise: `animateOnce` has no production caller in `packages/client-react/src` either — it is exercised only by its own unit test (`motion.test.ts:1-24`) — which matches its role as a prepared seam for one-shot entrance animations rather than a mechanism already wired into a panel; CSS transitions (technique 1) and FLIP (technique 2) currently cover every animation the shell actually ships.
...
> This section covers the shell's rendering of the auth lifecycle — the state
> machine, the gate, and the two overlays. For the full server-side flow
> (credential validation, token issuance/verification, the roster, and how to
> configure real credentials per platform), see
> [Authentication](../authentication.md).

**`AuthPresenter` — a genuine state machine, not a cosmetic toggle.** `AuthPresenter` (`packages/client-core/src/presenters/AuthPresenter.ts:34-172`) owns one `BehaviorSubject<AuthViewState>` whose state is `{ status, user, locked, error }` — `status` is `"unauthenticated" | "authenticating" | "authenticated"` (`:8,11-16`) — behind a `shareReplay({ bufferSize: 1, refCount: true })` (`:49-51`). Unlike the presenters compared elsewhere in this section, its constructor does real work: it calls `resume()` (`:55-70`), which reads the injected `SessionStore` and either restores an unexpired session as `"authenticated"` or clears a stale one and starts `"unauthenticated"` — so a page reload with a live session skips the login screen entirely. `login(username, password)` (`:73-84`) flips to `"authenticating"` and subscribes to the injected `AuthPort.login`, writing the resulting session to the store and flipping to `"authenticated"` on success or surfacing `describeAuthFailure(reason)` as `error` on failure (`:86-105,174-176`). `lock()` (`:108-116`) sets `locked: true` on an authenticated session (a no-op otherwise); `unlock(password)` (`:119-129`) re-runs the *same* `AuthPort.login` call for the current username — a genuine password re-check against the server/simulator, not a local toggle — refreshing the stored session and clearing `locked` on success, or setting `error` while staying locked on failure (`:131-150`). `logout()` (`:153-157`) clears the store and returns to the initial unauthenticated state. `createViewModel` exposes all four over one `useAuth()` — a plain `bind()` on `presenters.auth.state$` (`packages/react-bindings/src/createViewModel.ts:424-429`) plus stable command callbacks `loginAuth`/`unlockAuth`/`lockAuth`/`logoutAuth` that close over `presenters.auth` (`:432-446`) — returning `{ state, login, unlock, lock, logout }` (`:740-748`). `composition.ts` constructs it once as `new AuthPresenter(ports.auth, ports.sessionStore)` (`packages/client-core/src/composition.ts:299`), where `ports.auth` is `HttpAuthAdapter` against a real server or the domain's `AuthSimulator` in simulator mode — see [Authentication §2](../authentication.md#2-how-it-works-end-to-end) for the full credential-validation path on the other side of that port.

**`AuthGate` — the login gate `App`/`RootLayout` mounts around everything else.** `AuthGate` (`packages/client-react/src/ui/shell/auth/AuthGate.tsx:16-25`; RN analogue `packages/client-react-native/src/ui/shell/auth/AuthGate.tsx:14-23`) reads `const { state } = useAuth()` and renders `LoginScreen` in place of `children` whenever `state.status !== "authenticated"` — covering both `"unauthenticated"` and `"authenticating"` — and `children` (the whole app) once authenticated. `LoginScreen` itself (`packages/client-react/src/ui/shell/auth/LoginScreen.tsx:17-95`; RN `packages/client-react-native/src/ui/shell/auth/LoginScreen.tsx:26-110`) is a dumb form exactly like `LockScreen` below: username/password live in local `useState` only, are never logged, and `submitLogin` calls `login(username, password)` from the `useAuth()` seam; a non-null `state.error` renders inline (`data-testid="login-error"`), and the submit button disables while `state.status === "authenticating"`.

**`LockScreen` — render-`null`-unless-locked, the same overlay pattern as `BootGate`, with genuinely no local state.** `LockScreen` (`packages/client-react/src/ui/shell/lock/LockScreen.tsx:20-113`) reads `const { state, unlock } = useAuth()` and returns `null` outright when `!state.locked || !state.user` (`:20,26-28`) — there is no wrapping conditional at a call site the way `BootGate` conditionally renders `<BootSequence>`; the component itself is the gate. When locked, it renders the operator identity (initials, name, role, id) straight from `state.user`, the shared `HudLogo` component also used in `HeaderChrome` (`LockScreen.tsx:6,46-48`; `packages/client-react/src/ui/shell/chrome/HeaderChrome.tsx:3,39`), and two purely cosmetic components each headed by the same one-line comment, "DECORATIVE — cosmetic HUD readout, intentionally not wired to any port (spec: decorative-but-dead is allowed and explicit)" (`BiometricChannel.tsx:1`, `BiometricDots.tsx:1`): `BiometricDots` renders a fixed 4-of-6 lit/unlit dot row between the role line and the button (`BiometricDots.tsx:13-24`), and `BiometricChannel` renders the static `BIOMETRIC · ENCRYPTED CHANNEL` line below it (`BiometricChannel.tsx:12-18`) — both explicitly not backed by any port or live signal. The one live control is a real `<form onSubmit>` (`LockScreen.tsx:81-107`) whose `submitUnlock` calls `unlock(password)` from a local `useState` field (`:24,32-39`) — a genuine re-authentication request through the same `AuthPort` the initial login used, not a local `locked$.next(false)`; a non-null `state.error` renders inline as `data-testid="lock-error"` (`:94-98`).

**The trigger — the Account menu's lock and logout rows.** `AccountMenu` obtains its intents from the auth seam in one destructure, `const { state, lock, logout } = useAuth()` (`packages/client-react/src/ui/shell/chrome/AccountMenu.tsx:31`), and renders nothing at all when `!user` (`:33,36-38`). Its **⏻ LOCK SESSION** row — styled as the prototype's red Sign Out row — calls `lock()` directly (`:145-156`), raising `LockScreen` the same render cycle `App.tsx` next re-renders `state.locked` from the shared presenter; a separate **⏻ SIGN OUT** row below it calls `logout()` (`:160-171`), clearing the session and dropping straight back to `LoginScreen` via `AuthGate`. The same panel also reads the signed-in operator's full identity — name, email, trader id, desk, clearance (§3 of [Authentication](../authentication.md#3-the-four-roster-users--the-profile-dropdown)) — straight off `state.user` (`:83-113`).

*The lock overlay is the boot splash's structural twin in one respect only: both are dumb renderers of state that lives entirely outside the component, behind the same `useViewModel()` seam and the same render-nothing-unless-visible idiom. Unlike the splash, though, `AuthPresenter` is not purely local bookkeeping — `login`/`unlock` genuinely round-trip through the injected `AuthPort` to a real credential check (server or simulator), which is why this section, unlike §17.4's boot splash, links out to [Authentication](../authentication.md) for the other half of the flow. A third client swapping React for SolidJS ([§8.1](08-replaceability-matrix.md#81-the-multi-client-proof--the-solidjs-port)) reimplements only `LoginScreen`'s and `LockScreen`'s JSX — `AuthPresenter`, `useAuth`, and every byte of login/lock/unlock/logout orchestration carry over unchanged, exactly as they would for `BootGate`.*

### 17.6 The Equities Chart's Interaction Core

> Full design rationale — scope, in/out-of-scope tiers, and the alternatives
> considered — lives in
> [Equities Chart Interactivity — Design](../superpowers/specs/2026-07-26-equities-chart-interactivity-design.md).
> This section covers only the architecture that shipped.

**Hybrid mark shape, not one renderer for the whole chart.** The candle chart's existing rendering choice — axis-aligned rect marks (candle bodies, wicks, volume bars) as absolutely-positioned divs styled by CSS custom properties precomputed in `@rtc/motion-core` — is kept for exactly the reasons the design doc's §3.1 gives: it's the golden-locked renderer, it stays DOM-queryable for contract specs, and div-based glow/flash animations avoid the SVG-filter compositor trap `docs/performance.md` documents. Path marks — the line/area chart body, its gradient fill, and the SMA(20)/EMA(50) overlay polylines — get one sibling: a single absolutely-positioned `<svg viewBox="0 0 100 100" preserveAspectRatio="none">` overlay (`SvgPathLayer`) sharing the same coordinate space as the div marks beneath it, fed plain point arrays. `@rtc/motion-core` never emits markup for either shape — only numbers (rect geometries as CSS custom-property records, path marks as coordinate arrays) — so both shells (React's `SvgPathLayer`, Solid's counterpart) stay thin translators, not renderers with logic of their own. The crosshair follows the same rule as the rect marks: `CrosshairOverlay` is transform-positioned divs (`pointer-events: none`, so it never steals a gesture from the plot underneath), not a third SVG layer.

**The interaction math is pure, framework-blind, and lives in `@rtc/motion-core`.** Four modules — `chartViewport.ts` (pan/zoom clamping, live-edge follow/detection: `clampViewport`, `panBy`, `zoomAt`, `followLive`, `isAtLiveEdge`), `chartVm.ts` (candle/line/area geometry projection), `crosshairVm.ts` (pointer-fraction → snapped OHLC/volume/time readout), and `indicatorSeries.ts` (SMA/EMA polyline point arrays) — hold every piece of chart interaction logic as pure, unit-tested functions over structural stand-in types (no DOM, no framework import), per [ADR-005](../adr/ADR-005-ui-logic-placement.md). Each web client wraps them in its own thin gesture shell — React's `useChartGestures` hook, Solid's parallel `createChartGestures` — whose only job is translating wheel/pointer/keyboard events into calls against the pure viewport functions and holding the resulting `ChartViewport`/cursor state; the shells contain no interaction math of their own, matching the "thin event→pure-op translator" principle the design doc states at §2.

**`ChartPlot` — a presentational seam, not a gesture owner.** `ChartPlot` (`packages/client-react/src/ui/equities/chart/ChartPlot.tsx`, and its Solid counterpart) renders the plot's entire DOM tree — grid, price labels, marks, indicator overlays, crosshair, back-to-live affordance, volume pane, time axis — as a pure props leaf, extracted out of `CandleChart` deliberately so it has no dependency on `useChartGestures`/`createChartGestures` itself. In production, `CandleChart` owns the gesture hook and the `chartVm`/`crosshairVm`/indicator projections, and passes the live, gesture-driven `plotProps`/`plotRef` through. The visual test tier instead mounts `ChartPlot` directly with literal forced-state props (a panned viewport, an active crosshair position) — `plotProps`/`plotRef` are optional, and omitting both yields a static, gesture-free mount — because `useChartGestures`'s own state (mid-pan, mid-zoom, hovered) can never be reached deterministically through synthetic pointer events. The same production-vs-forced-state split that keeps `AuthPresenter`/`BootGatePresenter` swappable behind `useViewModel()` (§17.4, §17.5) shows up here one layer lower, inside a single component: the presentational tree and the thing that drives it are two separately testable pieces.

**The navigator strip is a second writer to the same `ChartViewport`, not a second source of truth.** The overview strip under the plot (`NavigatorStrip`, `packages/client-react/src/ui/equities/chart/NavigatorStrip.tsx` and its Solid counterpart) reads and writes the identical `ChartViewport` value `useChartGestures`/`createChartGestures` already own; the gesture hook grows exactly one new command for it, `applyViewport(vp)`, alongside the pan/zoom/keyboard writers it already had — a plain clamp-and-set, so every writer converges on the same `clampViewport` call regardless of which gesture produced the candidate viewport. The strip's window rectangle is never separate state: it is derived every render from that same viewport by `navigatorVm` (`@rtc/motion-core`), so the `followLive` live-edge slide described above moves the brush window for free — nothing downstream needs to know a brush exists at all.

**Three more pure functions cover the brush's own new math, staying in `@rtc/motion-core` alongside the plot's — plus one function the brush reuses unchanged.** `navigatorVm(series, viewport)` projects the *full* candle history (not just the visible window) into a 0–100 polyline plus the window's `--nav-left`/`--nav-w` CSS custom properties, in candle-slot-fraction space so the shaded region lines up with slot boundaries rather than candle centres. Resizing a handle calls `resizeViewportEdge(edge, vp, dCandles, seriesLen)`, which clamps only the moving edge directly into `[0, end − MIN_VIEWPORT_SPAN]` / `[start + MIN_VIEWPORT_SPAN, seriesLen]` — deliberately *not* routed through `clampViewport`, whose span-preserving clamp would slide the other, fixed edge once the drag reaches a series boundary, defeating the point of a one-sided resize. A track click (the strip outside both the window and its handles) calls `centerViewportAt(idx, vp, seriesLen)`, re-centring the window on the clicked index with the span held constant — here `clampViewport`'s span-preserving clamp is exactly the behaviour wanted, so it is reused as-is. Dragging the window's body reuses the plot's own `panBy` unchanged — the fourth function, not new — but with the delta's sign flipped relative to the plot drag: dragging the plot pulls earlier candles into view (the content follows the pointer), while dragging the navigator window moves the window itself toward the pointer (the window follows the pointer, because it represents the viewport rather than the content underneath it).

**The brush shells (`useNavigatorBrush`, `createNavigatorBrush`) push the ADR-005 "thin event→pure-op translator" shape to its logical zero.** Where `useChartGestures`/`createChartGestures` still hold some local reactive state (cursor, drag mode), the brush shells hold none: one drag-origin cell — a `useRef` in React, a bare `let` in Solid — cached at `pointerdown` (mode: edge-resize vs. body-move, plus the viewport the drag started from, so every `pointermove` recomputes from that fixed origin rather than accumulating onto an already-moving viewport, the same `DragOrigin` pattern the plot's own gesture hook uses), synthetic pointer handlers, and native `setPointerCapture`/`releasePointerCapture`. Zero effects, zero framework state cells in either shell — every value the brush touches is either the cached drag origin or the one shared `ChartViewport` passed in and handed straight back out through `applyViewport`, so there is nothing left for a `useEffect`/`createEffect` or a piece of local state to own.

**Backfill extends the series leftward through the same viewport, but the two growth directions fork.** `useChartGestures`'s render-adjust step now compares the series' oldest candle's time across renders as well as its length: when the series grew because older candles were *prepended* (the oldest timestamp moved earlier), the viewport is translated wholesale by the growth amount via `shiftForPrepend(vp, prependedCount)` — a plain both-edges shift, added beside `followLive` in `@rtc/motion-core/chartViewport.ts` — so a pan or zoom in progress survives untouched. A live tick appending a newest candle still takes the unchanged `followLive` path. Both forks share the same `grewBy = seriesLen - prevLen` arithmetic and diverge only on which edge moved, so "the series changed length" stays one guard with two consequences rather than two separate code paths.
...
Comparison series completes the tier: a `compare` symbol on the shared eqWorkspace machine overlays a second symbol's close-line on a percent axis — `chartScene`'s `compare` option derives a percent `ChartScale` (`base` = the first visible close, pct-range union back-converted to price units, series aligned by time), so drawings and the crosshair invert through the same scale unchanged, and clearing the comparison restores the stored linear/log choice. Backfill keeps the two series level through two sibling gates in `CandleChart`'s one fetch effect: the near-edge gate pages BOTH symbols while either can still grow (`onLoadOlder` → `loadOlderForChart`), and the catch-up gate — added when a late-activated comparison shipped rendering an incomplete line — fires while the compare's oldest candle is newer than the visible window's first candle, paging the compare ALONE (`onLoadOlderCompare` → `loadOlderForCompare`) until coverage or exhaustion, so a comparison activated or swapped after the primary already backfilled pages itself level without the primary fetching pages nobody scrolled to.
