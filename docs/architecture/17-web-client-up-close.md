...
*The lock overlay is the boot splash's structural twin in one respect only: both are dumb renderers of state that lives entirely outside the component, behind the same `useViewModel()` seam and the same render-nothing-unless-visible idiom. Unlike the splash, though, `AuthPresenter` is not purely local bookkeeping — `login`/`unlock` genuinely round-trip through the injected `AuthPort` to a real credential check (server or simulator), which is why this section, unlike §17.4's boot splash, links out to [Authentication](../authentication.md) for the other half of the flow. A third client swapping React for SolidJS ([§8.1](08-replaceability-matrix.md#81-the-multi-client-proof--the-solidjs-plan)) reimplements only `LoginScreen`'s and `LockScreen`'s JSX — `AuthPresenter`, `useAuth`, and every byte of login/lock/unlock/logout orchestration carry over unchanged, exactly as they would for `BootGate`.*
