...
Three DevTools footnotes keep the picture honest. The amber panel sections, the cells that size them, and the resize handles between them are host elements (`section.panel`, `div.cell`, `hr.handle`) emitted by `renderPanel` — a plain helper function, not a component (`InhouseLayoutEngine.tsx:515`) — so the components panel shows each head component and its `PanelErrorBoundary ▸ <panel>` pair sitting directly under a `SplitNode` (the one construct here that *is* a real component, extracted so its drag `useRef` obeys the rules of hooks, `InhouseLayoutEngine.tsx:253-257`), with no wrapper components in between. The four nav-tab buttons are likewise host `<button>`s from a `renderTab` helper inside `HeaderChrome` (`HeaderChrome.tsx:34-50`), not child components. And the two dashed sheets, `ConnectionOverlay` and `LockScreen`, are permanent leaves of `App` in DevTools but usually absent from the screen — each renders `null` until its presenter seam activates it (`ConnectionOverlay.tsx:15-17`, §17.5).
