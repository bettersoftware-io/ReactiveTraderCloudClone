...
**Problem.** Thirteen packages plus a `tests` workspace share a strict dependency graph (`domain` → `shared`/`ws-effects` → `client-core` → `react-bindings`/`solid-bindings`/`ui-contract` → clients (← `motion-core`)/`server`) that must build in topological order, and the single-runtime-dependency rule on `@rtc/domain` needs enforcement at install time, not just code review.
...
**Cost accepted.** Thirteen packages means thirteen `package.json`s, thirteen `tsconfig`s, and cross-package changes that ripple through the build graph in dependency order rather than landing in one file — more ceremony than a single-package repo for the sake of a graph the dependency rule can actually be enforced against.
...
**Choice.** Playwright Component Testing (`@playwright/experimental-ct-react`, bundled via Vite) screenshotted the UI against an injected fake `ViewModel`, writing committed PNGs under `packages/ui-contract/goldens/<tier>/__screenshots__/` — in-repo, not in a cloud diffing service — so the SolidJS port could point the *same* runner-shape at the *same* baseline directory and treat any pixel diff as a parity punch-list. This is exactly what shipped: `@rtc/client-solid`'s visual tier ran **assert-only** against these `react/` (and `react-local/<arch>/`) trees, generated only from `client-react` renders — it owns no golden images of its own, so a passing Solid run is a direct cross-framework pixel match. Two golden sets were committed per tier (canonical `react/` on pinned x86 CI, `react-local/<platform>-<arch>/` for local review) because screenshot rasterization is font/OS-dependent; a third tier (Vitest browser mode) diffed against the same goldens.
...
**Cost accepted.** Every intentional UI change had to regenerate *both* committed golden sets (the x86 set via a CI workflow, the local set via each runner's `:update` script) — no single command updated both, and `experimental-ct-react` itself was an unstable API surface across Playwright releases.

**Retired 2026-07-20.** A test-tooling bake-off measured all three candidate tiers and collapsed the assert role down to plain Playwright alone: Playwright Component Testing (`playwright-ct/`) was deleted outright along with its goldens, and Vitest browser mode (`vitest-browser/`) demoted to a coverage-only instrument (its pixel assert compiled out via `__RTC_VISUAL_SKIP_DIFF__`) rather than a second CI-asserted tier. `@rtc/client-solid` now asserts against the single surviving `playwright/` golden tree. Full verdict and measurements: [ADR-001's Outcome section](../../packages/client-react/tests/ui/visual/ADR-001-visual-diff-tooling.md#outcome-2026-07-20--the-test-tooling-bake-offs-visual-tier-verdict) and [§9.7](09-test-strategy.md#97-visual-golden-tiers).

### 10.10 A dedicated `tests` workspace running ten parallel e2e suites

**Problem.** Proving the behavioural-spec-survives-a-swap claim from [§1.2 rule 4](01-overview.md#12-architectural-principles) needs more than one driver exercising the specs — otherwise "framework-agnostic" is asserted, not demonstrated, and a driver-specific bug (a Playwright timing assumption) could hide inside the only implementation that runs.

**Choice.** `@rtc/tests` is its own top-level pnpm workspace member (`pnpm-workspace.yaml`), separate from any client or server package, orchestrating **seven suites** via `tests/scripts/run-all.ts`: five browser/presenter peers binding the same Gherkin `.feature` files through two different binding styles (Cucumber+Playwright, native Playwright — the pair duplicated against `@rtc/client-solid` — plus one presenter-direct peer, the plain-vitest winner of a 2026-07-20 runner/time-model bake-off), and two full-stack smokes that boot a real server ([§9.5](09-test-strategy.md#95-seven-suite-e2e-stack-4-browser-peers--1-presenter-peer--2-fullstack-smokes)). Keeping it a peer workspace rather than folding e2e into `client-react`/`server` lets it depend on built artifacts from *both* without either depending back on test tooling.

**Alternatives rejected.** A single browser driver (just Playwright, the CI-gating choice) was rejected as the sole e2e tier — the four presenter-direct peers exist specifically to prove the same specs validate the application layer with zero UI framework, closing the loop on the swap claim rather than assuming it. A second browser driver, Cypress, ran here too through 2026-07-19 (a native suite + a Cucumber-driven suite, mirroring the Playwright pair) as a live comparison for exactly this claim; a 2026-07-20 bake-off retired both suites — the forked scenario layer its command-queue model required, plus the arm64/Electron hazard, outweighed the portability proof once measured (see [`tests/STRATEGY.md`](../../tests/STRATEGY.md) §5.1).

**Cost accepted.** Ten suites is real CI wall-clock and maintenance surface — mitigated by `RTC_E2E_MAX_PARALLEL` and the presenter-direct fake-timer peers running ~19× faster than their real-timer counterpart, but every new scenario still touches multiple step-definition trees.
