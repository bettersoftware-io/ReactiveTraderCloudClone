...
| **Layout / panel manager**<br>in-house split-tree engine (`LayoutMachine` + `InhouseLayoutEngine`) + Dockview (`LayoutEngine` preference, default dockview) | ~1 dev-week per adapter — measured once (Dockview) | Engine branch in `WorkspaceEngine` + panel registries (content addressed by stable id) + `DockLayoutStore` (opaque persisted layout blob).<br>*Verified:* Shared `DockviewEngine.contract.spec.ts` (both clients) + `@rtc/layout-dockview` unit tests + e2e journey (switch engine -> drag-dock -> reload persists -> revert) |
...
**Deep dive:** [§21 One Test Suite, Two Frameworks — Cross-Framework Testing](21-cross-framework-testing.md) is the full synthesis of all three mechanisms behind this claim (contract swap-trio, assert-only visual tiers, `RTC_CLIENT_PKG` e2e), including the live scoreboard and the two real incidents each tier caught that the others couldn't.
