...
8. `packages/client-react/tests/ui/visual/react/registry.tsx` + `packages/ui-contract/src/visual/scenarios.ts` + `packages/client-react/tests/ui/visual/shared/fixtures.ts` — register the component and its visual-scenario fixtures so all three visual tiers can render it.
