...
6. `packages/ui-contract/src/shared/pages/<domain>/<name>/<Name>PanelPage.ts` + a registration line in `shared/components.ts` — the page-object and its wiring for the sociable-RTL contract tier. `packages/client-react/tests/ui/contract/` keeps only the `react/` trio (framework-specific render/registry glue) and the tier's configs.
7. `packages/ui-contract/src/specs/<domain>/<name>/<Name>Panel.contract.spec.ts` — the contract spec itself.
8. `packages/client-react/tests/ui/visual/react/registry.tsx` + `packages/ui-contract/src/visual/scenarios.ts` + `packages/ui-contract/src/visual/fixtures.ts` — register the component and its visual-scenario fixtures so the visual tier (and the vitest-browser coverage instrument) can render it.
...
6. `knip.json` — add a workspace entry **only if** the package has non-standard entry points (test globs, config files, a non-`src` root). Two existing packages, `client-core` and `react-bindings`, have no entry at all and rely on knip's default TS-library heuristics; `client-react`, `client-react-native`, `domain`, `shared`, `ws-effects`, `motion-core`, `server`, and `tests` all carry custom `entry`/`project` arrays because they have config files or non-standard roots. Run `pnpm lint:dead` after adding the package and only add an entry if it reports false-positive unused exports.
7. `.dependency-cruiser.cjs` **and `tsconfig.depcruise.json`** — dependency-cruiser is **not** auto-generic: its rules are hand-written per package name (regexes like `^packages/domain/src`), covering the workspace packages via at least one `pathNot`-anchored allowlist rule apiece (`to: { path: "^packages/", pathNot: "^packages/(<pkg>|<allowed>)/" }` — so a *new* package is forbidden by default until explicitly allowed, no stale blocklist to update). A brand-new package needs **two** wirings for its boundary to actually be enforced, and both are manual: **(a)** if it has an import boundary worth enforcing (a pure leaf, "must never import an app", etc.), add a `from`/`to` rule — nothing does it automatically; **(b)** add its `@rtc/<name>` **and** `@rtc/<name>/*` line pair to `tsconfig.depcruise.json`'s `paths`, which maps the package to its `src`. Skipping **(b)** is a silent trap: without it the cruiser resolves `@rtc/<name>` to the excluded `dist/` (or, in an unbuilt tree, to a bare unresolvable specifier), so **every rule touching that package matches nothing and enforces nothing**. See [dependency-cruiser.md](../dependency-cruiser.md) for the allowlist convention and the resolution rationale.
