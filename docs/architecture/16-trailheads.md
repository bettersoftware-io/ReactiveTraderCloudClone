...
3. `packages/client-react/src/ui/<domain>/<name>/<Name>Panel.tsx` + a co-located `.module.css` — the dumb component: `useViewModel()` for data, plain `data-testid="..."` literals in JSX are fine here (grep gate 1's "no raw `data-testid`" rule is scoped to the `tests/` package tree — it polices `tests/browser/page-objects/`, not `packages/*/src/ui`).
...
5. `tests/browser/page-objects/contracts/testids.ts` — add the panel's canonical test-id constant. This file **is** the one place gate 1 requires the literal to live.
...
- Grep gates 26–29 apply to every new file under `client-react/src/ui/**`: no `rxjs`/`@react-rxjs`/`@rx-state` import, no `localStorage`, no `fetch`/`import.meta.env`, no `setTimeout`/`setInterval` — all business/timer/transport/storage logic stays behind the ViewModel seam.
...
7. `.dependency-cruiser.cjs` — **not** auto-generic: its rules are hand-written per package name (regexes like `^packages/domain/src`) and today only cover `domain`, `shared`, `client-react`, `server`, and `ws-effects`. If the new package has an import boundary worth enforcing (e.g. "must never import react"), add a `from`/`to` rule — nothing does it automatically.
