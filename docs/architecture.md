...
Eight runners exercise the same behavioural surface via six binding styles. Cucumber-JS (with Playwright) and Cypress (via cypress-cucumber-preprocessor) bind Gherkin scenarios in `tests/specs/**/*.feature` to a shared step-definition tree. Native `@playwright/test` and native Cypress bind scenarios programmatically through their own step trees. Four presenter-direct peers — **cucumber-presenter** (real timers), **cucumber-presenter-fake-timers**, **vitest-presenter-fake-timers**, and **vitest-presenter** (plain) — bind a subset of the same scenarios (tagged `@presenter`) to the RxJS presenter layer in pure Node with no browser; the `cucumber` peer uses wall-clock waits, `cucumber-fake-timers` wraps the same bodies in `@sinonjs/fake-timers` virtual time, `vitest-fake-timers` reruns the same bodies under Vitest + the qpickle-loader Vite plugin for Gherkin + `vi.useFakeTimers()`, and `vitest` reruns the same `_shared/` scenario modules under Vitest + raw `describe`/`it` (no Gherkin loader) + `vi.useFakeTimers()` to prove the `_shared/*.ts` / `_await.ts` / `_world.ts` abstractions are useful even without a BDD step-tree. See Phase 5B.1, 5B.2, 5B.3, and 5B.4 specs for details.
...
| Presenter-direct harness | `tests/scenarios/presenter/_buildApp.ts` (App + simulator + test ConnectionEventsPort) · `tests/presenter/cucumber/{world,hooks}.ts` |
...
| Presenter-vitest-fake-timers runner | `tests/presenter/vitest-fake-timers/vitest.config.ts` · `vitest` + the qpickle-loader Vite plugin + `vi.useFakeTimers()` — same 20 `@presenter` scenarios under Vitest |
| Presenter-vitest-fake-timers harness | `tests/presenter/vitest-fake-timers/{world,hooks,setup}.ts` (VitestFakePresenterWorld implements the same `AwaitHelpers` interface as the cucumber peers; `setup.ts` barrel loaded via `vitest.config.setupFiles`) |
| Presenter-vitest-fake-timers step defs | `tests/presenter/vitest-fake-timers/steps/*.steps.ts` — functional mirrors of cucumber steps with loader import, world-type swap, and `async (state:)` callback shape |
| Presenter-vitest (plain) runner | `tests/presenter/vitest/vitest.config.ts` · `vitest` + raw `describe`/`it` (no Gherkin loader) + `vi.useFakeTimers()` — same 20 `@presenter` scenarios as the other 3 presenter peers |
| Presenter-vitest (plain) harness | `tests/presenter/vitest/_world.ts` (VitestPlainPresenterWorld plain-object factory implementing the same `AwaitHelpers` interface; one `*.test.ts` per feature, beforeEach/afterEach building/tearing down the world per `it()`) |

**Bundler-alias seam (Cucumber+Cypress).** `tests/browser/steps/*.steps.ts` files unconditionally `import { Given, When, Then } from "@cucumber/cucumber"`. Cucumber-JS resolves this natively in Node. Cypress's esbuild bundler (configured in `tests/browser/cypress-cucumber/cypress.config.ts`) installs a 5-line plugin that intercepts the specifier and remaps it to `@badeball/cypress-cucumber-preprocessor`. Both packages expose API-compatible `Given/When/Then/And/But/defineParameterType` decorators, so the same call sites compile cleanly under either resolution. The trick is invisible at the step-file level. Hooks and `World/setWorldConstructor` are NOT shared — they live in the per-runner `tests/browser/{playwright-cucumber,cypress-cucumber}/` directories.
...
| 15 | No driver imports in presenter step/scenario/support files (excludes the vitest-fake-timers peer) |
...
| 20 | No Gherkin loader imports (`quickpickle`, `@cucumber/cucumber`) inside `tests/presenter/vitest/` |
| 21 | `@presenter` scenario count per `.feature` file must equal `it()` block count in the matching `vitest/*.test.ts` file (custom check) |
| 22 | Every `describe(...)` title in `tests/presenter/vitest/` must begin with `"@presenter Feature: "` |
...
| 24 | `vitest-fake-timers/setup.ts` must import every step file in `tests/presenter/vitest-fake-timers/steps/` (barrel completeness) |
