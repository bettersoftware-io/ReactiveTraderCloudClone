...
| Presenter-fake runner | `tests/cucumber-presenter-fake.js` · `@cucumber/cucumber` + `@sinonjs/fake-timers` — same 20 `@presenter` scenarios under virtual time |
...
| Presenter-vitest runner | `tests/vitest-presenter-fake.config.ts` · `vitest` + the qpickle-loader Vite plugin + `vi.useFakeTimers()` — same 20 `@presenter` scenarios under Vitest |
...
| Presenter-vitest step defs | `tests/steps/presenter/vitest-fake/*.steps.ts` — functional mirrors of cucumber-real steps with loader import, world-type swap, and `async (state:)` callback shape |
| Presenter-vitest-plain runner | `tests/vitest-presenter-plain.config.ts` · `vitest` + raw `describe`/`it` (no Gherkin loader) + `vi.useFakeTimers()` — same 20 `@presenter` scenarios as the other 3 presenter peers |
