# Reactive Trader Redesign — Developer Handoff (v4)
...
- **Ambient background** (holo/neon): two styles behind a preference — `Aurora`
  (northern-lights curtain layers, fixed green/teal/purple palette, keyframes
  `auroraA`–`auroraE`) and `Rays` (original accent blobs + radar sweep). Pref key
  `prefs.ambientStyle`; both gated by the Animated background toggle and `--auroraOp`.
...
| # | key | function | scene |
|---|---|---|---|
| 1 | `core` | `_drawBoot` | global market mesh: globe drawn in by meridian sweeps, gyro rings, 10 hub nodes w/ ping ripples + callouts, buy/sell order arcs |
