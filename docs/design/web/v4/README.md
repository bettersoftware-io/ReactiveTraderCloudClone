# Reactive Trader — Futuristic Redesign Prototype (v4)
...
Equities module + Admin observability dashboard, and a rotation of **eight cinematic 3D
boot sequences** (Jarvis-hologram inspired), several of them cursor-tracked.
...
This folder contains two things (plus `CHANGELOG.md` listing what changed since v3):
...
This folder lives at `docs/design/web/v4/`, alongside the earlier web
iterations `web/v1/`–`web/v3/` (the mobile prototype lives under
`docs/design/mobile/`). It is self-contained and has no build step.
