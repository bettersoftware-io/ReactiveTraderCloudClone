...
| Phase 4 — Reorganise `packages/client-react/src/` into `app/` + `ui/` subtrees | ✅ DONE | `plans/2026-05-07-phase-4-app-ui-reorg.md` | `dd84f6a..10861fe` (10 task commits) + this STATUS update |
...
## 2026-06-07 — client visual tier moved in-suite (Part A of the test-report consistency spec)

Scripts and folder renamed to the full-information name per the naming rule; tier
configs moved in-suite; the stray root-level `playwright/` CT host template is now
the suite's `playwright-ct/host/` (explicit `ctTemplateDir`), symmetric
...
(The tier was later folded under the `tests/ui/` umbrella alongside the contract
tier — it now lives at `tests/ui/visual/`, with scripts `test:ui:visual*` and
the `@ui-visual` import alias.)
...
4. **`@rtc/client-react` package exports.** 5B.1 added a top-level barrel + `exports` field + `tsconfig.types.json` for declaration emission. Other tooling that imports `@rtc/client-react` (Vite build, server, etc.) wasn't affected, but verify on the next major dep bump.
...
1. **react-rxjs version + dependency**: `rxjs ^7.8` is already in `@rtc/client-react`. `react-rxjs` to be added — confirm latest version + interop with React 19.
...
3. **Composition Root location**: replaces `packages/client-react/src/services/service-provider.tsx`. Options:
   - New file `packages/client-react/src/app/composition.ts` (runs at startup, returns wired-up presenters).
