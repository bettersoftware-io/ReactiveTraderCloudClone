...
**Architecture:** The two *data-driven* visual tiers (`playwright/visual.spec.ts`, `vitest-browser/visual.spec.tsx`) already loop the scenario manifest. We add a **theme-matrix expander** in `shared/scenarios.ts` that REPLACES each base scenario with its full 10-combo cross-product (5 skins × dark/light), each carrying an explicit `themeSkin`/`themeMode` that `VisualScenario.tsx` layers onto the fixture's `AppData` before building the fake ViewModel. A shared `goldenPath`/`goldenPathArray` routes each golden to a `<skin>-<mode>/` sub-folder. The **`playwright-ct` tier is rewritten to be data-driven too** — one `matrix.spec.tsx` looping `scenarios` (mirroring the plain-Playwright tier's body via `scenarioActionFor`) replaces the 15 hand-written per-component specs — so all three tiers derive identically from the shared manifest and every combo is captured in every tier. Canonical `react/` goldens are baked inside the CI Playwright container via `update-visual-goldens.yml`; the per-arch `react-local/` set is generated on the dev machine (darwin-arm64 here; linux-arm64 in the aarch64 sandbox).
...
- **Viewport 1920×1080 (user-confirmed).** All three tier configs render at a realistic 1080p desktop (was 1280×720 / vitest 1280×800). The HUD is `height:100vh`, so this viewport IS the full-page golden size; 1280×720 cramped the 4-panel HUD. Component (scenario-root) shots are content-sized and largely unaffected.
...
- **All three tiers get the full matrix.** playwright-ct, playwright, vitest-browser each capture all 1222 scenarios (122 expandable base × 10 combos = 1220, plus the 2 un-expanded `MATRIX_EXCLUDE` mode-cycle scenarios). No bare base keys remain. CT is rewritten to data-driven for this (Task 3).
