...
- [ ] **Step 1:** Failing choreography tests, driving the effects with an `Inbound` stream the way `fx.effects.test.ts` does: (a) `jarvis.chat` ("where is EURUSD?") → outbound sequence `jarvis.toolEvent(running)` … `jarvis.delta`× … `jarvis.toolEvent(done)` … `jarvis.done`, with the deltas reassembling the quote reply **(CORRECTED post-review: actual order is `toolEvent(running)` → `toolEvent(done)` → deltas → `done` — the badge brackets the tool call, not the speech; the shipped test in `jarvis.effects.test.ts` asserts this order)**; (b) trade turn → `jarvis.confirmRequest` (payload has `confirmationId`, `symbol`, `direction`, `notional`, `quotedPrice`, `ratePrecision`, no `type` field) → send `jarvis.confirm {approved: true}` → fill reply + `jarvis.done`, trade visible in the container blotter; (c) `{approved: false}` → declined copy; (d) tearing down the chat stream mid-confirmation then confirming is a no-op (no execution) — the engine's `turnConfirmationIds` teardown, proven at the wire layer.
