...
- **`AsyncStoragePreferencesAdapter`** (`src/app/adapters/AsyncStoragePreferencesAdapter.ts`) — implements `PreferencesPort`. The port's `*$()` streams are *replay-current* (a subscriber must receive the current value synchronously on subscribe — the contract that prevents a theme/boot flash). AsyncStorage is **async**, so the adapter: on construction, seeds each axis's `BehaviorSubject` with a default, then asynchronously hydrates from AsyncStorage and pushes the stored value; each `set*` writes through to AsyncStorage and pushes to the subject. Backed by `@react-native-async-storage/async-storage` (Expo-Go-bundled). ~50–70 lines; unit-tested against a mocked AsyncStorage and the existing `PreferencesPortContract` describer.
...
- **`SpotTile`** — one currency pair: symbol, bid & ask, spread, and a ▲/▼ directional flash on tick (up/down/unchanged), sourced from `PriceStreamPresenter`'s per-symbol stream. The price-formatting + direction logic is reused from `client-core` (the presenter/domain), not re-implemented.
...
2. **RN contract tier (new pillar):** a `react-native/` swap of a framework-neutral `SpotTile.contract.spec` driven by `@testing-library/react-native`. The SAME neutral behavioral spec that validates the React leaf validates the RN leaf — the third portability implementation (`react/` · `react-native/` · future `solid/`). This stands up the RN test harness and is the phase's main new test infrastructure.

   **Known risk (the phase's #1 infra unknown — the Metro-equivalent).** `@testing-library/react-native` is historically jest-based, while this repo standardizes on **vitest**. The plan must resolve the harness up front: prefer running `@testing-library/react-native` under vitest with the `react-native` transform/preset (keeps one runner repo-wide); if that proves intractable under Node 26 + SDK 55, fall back to a scoped **jest island** for RN component tests only (the neutral `*.contract.spec` and node-env unit tests stay on vitest). Whichever path, it must integrate with the repo gates (biome/eslint/knip/typecheck) exactly like every other test tier. This is a fail-fast checkpoint: if the harness can't be stood up cleanly, surface it before building the leaves against it.
...
| SpotTile contract spec — `react-native/` swap | controller | CI (contract tier) |
