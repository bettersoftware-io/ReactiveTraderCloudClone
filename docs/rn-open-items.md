...
| **T13** | **`HologramScene` throws `TypeError: undefined is not a function` on device, repeatedly.** Observed 2026-07-27 on iPhone 17 / iOS 26.5, dev client against Metro: the error fires continuously with `HologramScene` as the only stack frame, while the rest of the app runs normally. | **OPEN, and NOT introduced by 5c** — nothing in Phase 5c touches the boot scenes; this is already on `main` from #382 (6b-2a). Almost certainly the recurring **worklet class** (T11, #334, #340, and the `craftGridLines` crash in #353): a function called from inside the draw worklet without a `"worklet"` directive resolves to `undefined` there. jest cannot see it — the scene's 40 tests pass — so the simulator is the only witness, exactly as recorded for the other four projected scenes. Note the scene's own header comment claims `hologramColumns()`/`hologramHexCells()`/`hologramMotes()` are called in a `useMemo`; they are called bare in the render body (`HologramScene.tsx:115-117`). That is React-land either way so it is not the crash, but the comment is wrong and the next reader will trust it. Diagnose by bisecting the draw helpers for a missing directive. |
