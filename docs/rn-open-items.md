...
| **T9** | **The Maestro tier is stale and nothing notices.** `tests/visual/maestro/flows/` holds **3** committed flows (`blotter_seeded`, `shell_appearance`, `shell_connection-banner`) against **8** `SCENARIO_IDS`, and only 3 of 8 `maestro` goldens exist — but `maestro/run.ts` iterates all 8, so a full run throws on the first missing shot. `generateFlows.test.ts` exercises only the pure `flowYaml()` function and never asserts that the *committed* flows match `SCENARIO_IDS`, so the drift is invisible to CI. | Open, found 2026-07-26 while scoping T1. Same class as the parked-Gherkin step-tree drift: a generated artifact committed alongside its generator, with no test tying the two together. Regenerating flows is one `tsx generateFlows.ts`; the 5 missing goldens need a Mac-local `:update` pass plus an eyeball, so this cannot be closed from CI. The durable fix is a test that fails when the committed flow set diverges from `SCENARIO_IDS`. |
