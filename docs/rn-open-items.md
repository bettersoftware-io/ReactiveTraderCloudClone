...
| Item | State |
|---|---|
| **Phase 5** (Credit / Equities / Analytics) | **5c (Analytics) is BUILT except its on-device sign-off** — simulator drift (M6), formatters, Skia P&L chart, animated pair bars, Skia exposure bubbles with breathing, prototype card order, and an `analytics/dashboard` visual scenario. Plan: [5c](superpowers/plans/2026-07-26-rn-mobile-v1-rehaul-phase-5c-analytics.md). **5a (Credit) and 5b (Equities) are designed but have no plan files and no implementation**; 5b is the larger of the two. |
...
| **T13** | **FIVE of the eight boot scenes render BLANK on device** — `hologram`, `layers`, `geo`, `jarvis`, `topo`. A white screen plus a continuous `TypeError: undefined is not a function`; one session logged 253. The live boot splash is affected too, not only the harness: a cold launch into the persisted variant paints white for its whole duration. Observed 2026-07-27, iPhone 17 / iOS 26.5. | **OPEN — root cause PARTIALLY identified, NOT fixed.** Not a 5c defect; on `main` since #382/#383/#390-#393, each merged as "not verified on device". **Proven:** a worklet cannot call a worklet declared LATER in the same file — the transform captures the closure by value at module evaluation, so the later sibling is still `undefined`; imports are immune. Demonstrated causally by moving one helper above its caller and watching it flip while its still-below sibling did not. **Not proven:** that fixing this fixes the scenes — `hologram`'s single forward reference was reordered, the detector went clean, and it STILL renders blank. So there is at least one more cause. Full write-up, including four DISPROVEN theories worth not re-running (deprecated `SkPath`, removed `MakeDash`, cross-package worklets, double-applied Babel plugin): [research/2026-07-27-rn-boot-scenes-blank-on-device.md](research/2026-07-27-rn-boot-scenes-blank-on-device.md). `scripts/check-worklet-order.mjs` reports the forward references (23 across 6 files) and reproduces the 5-broken/3-clean split exactly — necessary, not sufficient, so deliberately NOT a CI gate yet. |
| **T14** | **The boot visual goldens were captured on a WHITE background.** `BootSceneFixture` mounts a `<Canvas>` with no backdrop and `VisualScenarioHost`'s wrapper sets no `backgroundColor`, so every `boot/*` golden shows its scene over white. The app's own boot chrome is dark, so this is a harness gap, not a product defect. | **OPEN.** Noticed while shooting `core` on 2026-07-27 — the globe renders correctly but floats on white. Consequence worth stating plainly: #353 recorded these goldens "reproducing at 0.01-0.05%", which was true and meaningless — they reproduce a wrong background faithfully. Reproducibility is not correctness. Fix is one `backgroundColor` on the fixture, then re-shoot; do it in the same session as T13's fix so the scenes are shot once. |
...
| **T12** | **No golden exists yet for `analytics/dashboard`, or for any of the five projected boot scenes.** The scenario is registered and its Maestro flow generated, but nothing has been captured. | **OPEN by choice, not oversight.** Golden capture and on-device sign-off are batched into one simulator session after every UI-changing phase lands, so the goldens are shot once rather than re-shot after each phase. Two things this scenario needed, worth reusing: data pinned by mounting `AnalyticsDashboard` (the presentational split) over a literal book, and `VisualScenarioHost`'s new `powerSaverLevel="freeze"` seed to hold the bars' and bubbles' entry tweens at rest. **`forceReduceMotion` does not cover widget tweens** — it seeds `animatedBackground`, which gates the ambient layer alone. Any future scenario containing motion-gated widgets needs the power-saver seed too. |
