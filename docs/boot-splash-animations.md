...
`rt_bootSeq`). Because it's in `localStorage`, the pointer survives a real page
reload, not just a React remount. `BootPreferencePresenter.current()` reads it
synchronously (safe: the port is `BehaviorSubject`-backed and emits its current
value on subscribe).
