...
- **Classic-skin fonts are host-environment-sensitive in CT visual crops** — classic is the only theme skin whose font tokens resolve to OS-generic keywords (`system-ui`/`ui-monospace`) rather than embedded `@fontsource` faces, so element-cropped screenshots of text-width-driven surfaces vary with the runner's system fonts. Surfaced by the SolidJS-port Phase 4 assert-only tiers (PR #230): GitHub-hosted runners resolve these differently than fresh pinned-image containers, so 4 classic CT assertions (chrome/header + admin/event-log × dark/light) are skipped ON CI ONLY in solid's ct-fallback tier (still verified by the other two x86 tiers and all darwin tiers). Robust fix, if wanted: back classic's font tokens with a deterministic embedded face — a design change (subtly alters classic's rendering) needing sign-off.
- **Power-saver UI on RN** — power-saver mode shipped for the web React client (PR #218; ref [power-saver-mode.md](power-saver-mode.md)) and the Solid client (PR #230, SolidJS-port Phase 4). The RN client **persists** the `powerSaver` preference via its adapter but has **no UI** for it — no header ⌁ toggle, no ambient-layer gating, no `--fx-play` motion pausing. Fold the RN UI into the RN mobile-v1 rehaul. Not spec'd yet.
