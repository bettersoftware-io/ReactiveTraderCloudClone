...
| T12 | **Solid:** reference-keyed `<For>` over a vm array that is *recreated* per update | `<For>` keys by object identity — fresh objects every update mean the whole collection's DOM is disposed and recreated per change (plus a forced layout per event from the structural churn). Invisible on fast hardware; 3.45× React's drag cost under 8× CPU throttle. Use `<Index>` (position-keyed, in-place binding updates) for windowed/per-frame-derived collections. | solid chart candles/grid/labels/volume/time-axis/depth ladder ([react-vs-solid-performance.md](react-vs-solid-performance.md) finding 3) |
| T13 | **Solid:** unthrottled per-pointermove signal writes during continuous gestures | Solid has no input-pressure scheduler: every event renders synchronously, so a slow machine renders stale positions late (the "sluggish drag" feel). Coalesce continuous writes leading-edge + trailing-frame (latest wins) — a no-op on fast hardware, React-style load-shedding on slow. | solid `createChartGestures.writeViewportThrottled` |
