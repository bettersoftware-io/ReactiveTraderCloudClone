...
packages/
  ...
  core-logic/          @rtc/core-logic          — The rules all three application cores share that need no stream library (slice 8): the pure folds and view derivations, the workspace and Jarvis controllers (`createWorkspaceDock`, `createJarvisController`, `applyDriveCommand`, …), `createAuthDeps`. Runtime deps domain + shared only (core-api for types); `core-logic-stays-pure` / `core-logic-stays-inner`. Re-exported whole by client-core.
  client-core/         @rtc/client-core         — Framework-free application core (the RxJS one, the default), implementing `@rtc/core-api`: composition root, presenters, state machines, WsAdapter + port factories (which also supply `connectionIntentsPort`). Depends on core-api, core-logic, domain, shared (+ rxjs, @rx-state/core). No React/DOM/RN imports.
  client-core-async/   @rtc/client-core-async   — Sibling application core on async/await + AsyncIterable: a Store/Topic/spawn/sleep kernel, with `bridge/` the only place rxjs is imported as a value; `kernel/runSlot.ts`'s `createRunSlot` is the shared "one active run" guard every superseding machine uses. All 74 members native, ported slice by slice (1a–7); since slice 8 it stands alone — `createApp` builds only its own members, `dispose()` releases every port subscription, and it gates `ports.transport` on its own `auth`. Depends on core-api, core-logic, domain, shared (+ rxjs, @rx-state/core as types outside bridge/); client-core is a devDependency for test adapters only (`alt-cores-no-client-core-at-runtime`).
  client-core-effect/  @rtc/client-core-effect  — Sibling application core on Effect-TS (`effect` ^3.22.2): Context/Layer composition root over a ManagedRuntime (one runSync resolves the native presenters), Stream/SubscriptionRef bridge; `machines/runSlot.ts`'s `createRunSlot` is the shared run-token guard. All 74 members native (the Jarvis family and its workspace built outside the Layer graph on child hosts, with a synchronous bridge `createHotStream` for their hot events); since slice 8 it stands alone (`composeApp` → `{ app, host }`; `dispose()` closes the host scope, then the runtime) and gates `ports.transport` on its own `auth`. Depends on core-api, core-logic, domain, shared, effect (+ rxjs as types outside bridge/); client-core is a devDependency for test adapters only.
...
**Application core rule:** `@rtc/core-api` is types-only — it exports no runtime value (grep gate 42) — and is the contract all three cores implement: `@rtc/client-core` (RxJS, the default) and its two siblings, `@rtc/client-core-async` and `@rtc/client-core-effect`. The rules they share that need no stream library live in `@rtc/core-logic` (runtime deps domain + shared only; `core-logic-stays-pure` forbids a runtime rxjs import there). Outside each sibling's own `bridge/` directory, `rxjs`/`@rx-state/core` may be imported as **types only**; an operator or a live value import anywhere else is a lint error (dependency-cruiser `bridge-owns-rxjs` + grep gate 43) — an "alternative core" that reaches for `shareReplay` is RxJS with extra steps, not a second implementation. All 74 members are native in both siblings (ported slice by slice, 1a–7), and since slice 8 (2026-09-26) each stands alone: no base app, no `CoreSeams`, no parity manifest — each core's native presenter map is typed exact (`Omit<Presenters, …>`), so the typecheck is the completeness witness, and `@rtc/client-core` is a sibling's devDependency for test adapters only (`alt-cores-no-client-core-at-runtime`). A core reaches the connection-event stream only through `AppPorts.connectionIntents` (never a module-level Subject), and gates `ports.transport` on its own `auth` (the `transportGate` contract suite). Behavioural equivalence is witnessed by `@rtc/core-contract` (one suite per member plus the cross-member `portDiscipline`, with absolute construction-time counts, and `transportGate`); `pnpm check:core-bundle` proves each web build ships exactly one core, in every direction. See [ADR-006](docs/adr/ADR-006-pluggable-application-core.md) and `docs/architecture/22-pluggable-application-core.md` (§22).
