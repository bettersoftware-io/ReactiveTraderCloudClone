...
packages/
  ...
  server/              @rtc/server              — Native WebSocket + @rtc/ws-effects (24 effects: FX/Credit/Admin/Equities, plus the JARVIS_* effects). Depends on domain, shared, ws-effects, agent-tools (+ ws, rxjs, and `@anthropic-ai/sdk` confined to `src/agent/` — the only package in the repo allowed to import it).
```
