...
Monorepo with pnpm workspaces + Turborepo; twenty-four packages plus the `tests` workspace. All packages build, typecheck, and pass tests. Three shipping web/mobile clients (React, RN/Expo, SolidJS) share the framework-free `@rtc/client-core`. `@rtc/client-solid` is at full parity with `@rtc/client-react`: the same shared `@rtc/ui-contract` behavioural specs pass against both (via a per-framework swap-trio), its visual tier asserts pixel-for-pixel against the goldens in `@rtc/ui-contract` (generated only from `client-react` renders; assert-only — `client-solid` owns none of its own), and the same Gherkin e2e suites run against it — built on the parallel `@rtc/solid-bindings` bridge, the `@rtc/react-bindings` sibling. A custom devtools trio (`@rtc/devtools-core` + `@rtc/devtools-app` + `@rtc/devtools-extension`, plus the standalone `@rtc/devtools-relay` for RN) gives the non-Redux state layer live state inspection through a store-first navigation tree (Redux DevTools' mental model — All / Presenters→streams / Machines→kind→instance / Wire→msgType, scoped actions list, Clear as Commit), dormant until an inspector attaches — served same-origin at `/devtools/`, or attached to any running app (including the deployed build) via the MV3 Chrome extension. The application core is pluggable as of slice 0, with all 74 members native in both alternative cores as of slice 7 — connection, every preference presenter, `commands.reconnect`, the FX pricing/blotter/execution presenters and their five machines, the credit presenters (`rfqs`, `dealers`, `instruments`, `rfqQuote`) and the four RFQ machines, the equities presenters (`watchlist`, `candleSeries`, `depth`, `ordersBlotter`, `positions`), the two equities workspace singletons and `orderTicket`, the nine admin members (`throughput`, the three metric windows, `topology`, `eventLog`, `sessions`, `sessionsKpi`, `incident`), five shell members (`auth`, `bootGate`, `workspaceNav`, `animationDirector`, the `boot` machine), the twelve workspace members (the layout machines, the dock bridges, `jarvisPanels`, `layoutPresets` and kin) and the Jarvis family (`jarvis`, `jarvisUsage`, `jarvisDriver`, `jarvisDemo`, the narrator) — (`@rtc/core-api` + `@rtc/client-core-async` + `@rtc/client-core-effect`, still composed beside a stood-down `@rtc/client-core` base app until slice 8 removes the delegation, selectable via `VITE_CORE_IMPL`) — see `docs/STATUS.md` for the remaining vertical-slice backlog. `docs/architecture.md` is the authoritative architecture reference.
...
pnpm dev:watch   # Rebuild-watch every pure-TS library (domain, shared, ws-effects, motion-core, ui-contract, devtools-core, core-api, core-contract, client-core-async, client-core-effect) — run alongside a client to hot-rebuild lib edits
...
pnpm dev:react:async     # @rtc/client-react on the async/await + AsyncIterable application core (sim mode)
pnpm dev:react:effect    # @rtc/client-react on the Effect-TS application core (sim mode)
pnpm dev:solid:async     # @rtc/client-solid on the async/await + AsyncIterable application core (sim mode)
pnpm dev:solid:effect    # @rtc/client-solid on the Effect-TS application core (sim mode)
# VITE_CORE_IMPL composes with any transport mode above, e.g. `VITE_CORE_IMPL=effect pnpm dev:react:ws:local`
...
**TypeScript toolchain — a deliberate dual install (since 2026-09-13).** `tsc` is TypeScript **7** (the Go-native compiler, linked from the `@typescript/native` alias) while the `typescript` package name resolves to the **6.x JS API** (`npm:@typescript/typescript6`), because TS 7.0 ships no programmatic API and three CI gates need one (`typescript-eslint`, `dependency-cruiser`, Expo's `app.config.ts` loader). Consequences that look wrong but are correct: `pnpm outdated` / Renovate showing `typescript` at 6.x; a `tsc6` bin nothing calls (it is for A/B repros); `tsconfig.depcruise.json` keeping `baseUrl`, an option `tsc` 7 rejects but only the 6.x API ever reads. Whole-repo typecheck went 21.9 s → 4.7 s sequential. Measurements, blockers and the exit plan: `docs/typescript-7.md`; the move-off-6 item lives in `docs/STATUS.md`. **Never run two builds in one checkout at once:** 13 packages use project references, so a `--filter`ed `tsc --build` of one package also rebuilds the libraries it references, and two such builds can truncate a shared `.d.ts` to zero bytes — tsc's incremental build info then marks it emitted and turbo caches it, surfacing later as `TS2306 … is not a module` on a file nobody touched. Every tsc-emitting `build` therefore ends with `scripts/check-dist.mjs` (enforced by `pnpm check:scripts`), which fails THAT build by name; `pnpm check:dist` runs it over every package, and its message carries the recovery recipe (delete the package's dist + build info, rebuild it alone, `turbo run build --filter=… --force`).
...
|---|---|
| `/rtc:gauntlet [full]` | Local mirror of CI's `checks` job. Bare = the 20 fast gates (~50s, no build). `full` adds typecheck, unit tests, the four ≥95% coverage gates, type-aware ESLint, the lint-warnings ledger, build, and the post-build `/devtools/` check (~8 min). `e2e` is excluded — it's a separate CI job; run `pnpm test:e2e` explicitly. |
...
packages/
  ...
  core-api/            @rtc/core-api            — Types-only application-core contract (ADR-006, §22): Stream<T>/StateStream<S> aliases, one interface per presenter, machine state/intents/view types, Presenters/MachineFactories/AppCommands/AppPorts/App, CoreFactory. Exports no runtime value (grep gate 42). Innermost after domain/shared.
  client-core/         @rtc/client-core         — Framework-free application core, implementing `@rtc/core-api`: composition root, presenters, state machines, WsAdapter + port factories. Depends on core-api, domain, shared (+ rxjs, @rx-state/core). No React/DOM/RN imports.
  client-core-async/   @rtc/client-core-async   — Sibling application core on async/await + AsyncIterable: a Store/Topic/spawn/sleep kernel, with `bridge/` the only place rxjs is imported as a value; `kernel/runSlot.ts`'s `createRunSlot` (slice 4) is the shared "one active run" guard every superseding machine now uses. Slice 4: forty-four members native (connection, all fifteen preference presenters, commands.reconnect, the six FX pricing/blotter presenters, the five FX machines, the four credit presenters, the four RFQ machines, the five equities presenters, the two equities workspace singletons, orderTicket); slice 5: the nine admin members, `kernel/foldTopic.ts` the warm-fold primitive; slice 6: five shell members; slice 7 wave 1: the twelve workspace members over client-core's shared rxjs-free workspace controllers; slice 7 wave 2: the Jarvis family over the shared Jarvis controllers — all 74 native (the base RxJS app is still composed, stood down via `CoreSeams.nativeJarvis`, until slice 8); a committed parity.json + reference-inequality drift test track native-vs-delegated provenance. Depends on core-api, client-core (strangler-phase only), domain (+ rxjs, @rx-state/core as types outside bridge/).
  client-core-effect/  @rtc/client-core-effect  — Sibling application core on Effect-TS (`effect` ^3.22.2): Context/Layer composition root over a ManagedRuntime (one runSync resolves the native presenters), Stream/SubscriptionRef bridge; `machines/runSlot.ts`'s `createRunSlot` (slice 4) is the shared run-token guard every superseding machine now uses. Slice 7: all 74 members native (slice 4's forty-four, slice 5's nine admin members, slice 6's five shell members, slice 7's twelve workspace members and the four Jarvis members — the Jarvis family and its workspace built outside the Layer graph on child hosts, with a synchronous bridge `createHotStream` for their hot events). Same strangler shape and parity manifest as client-core-async. Depends on core-api, client-core (strangler-phase only), domain, effect (+ rxjs as types outside bridge/).
  core-contract/       @rtc/core-contract       — Dev-only behavioural equivalence tier over Presenters/MachineFactories (ADR-006 Decision 5, §22): exhaustive CONTRACT_SUITES registry keyed by member + PENDING_SUITES drift test, a scripted-AppPorts harness, one runner file per core. Depends on core-api, domain (+ rxjs) only — never client-core, to avoid a build-order cycle. Consumed by every core as a devDependency, never from src.
...
**Application core rule:** `@rtc/core-api` is types-only — it exports no runtime value (grep gate 42) — and is the sole shared surface between `@rtc/client-core` (the RxJS implementation) and its two siblings, `@rtc/client-core-async` and `@rtc/client-core-effect`. Outside each sibling's own `bridge/` directory, `rxjs`/`@rx-state/core` may be imported as **types only**; an operator or a live value import anywhere else is a lint error (dependency-cruiser `bridge-owns-rxjs` + grep gate 43) — an "alternative core" that reaches for `shareReplay` is RxJS with extra steps, not a second implementation. Both siblings port members slice by slice (all 74 native in both as of slice 7 — the 74th, `layoutPresets`, arrived delegated with Dockview Phase 6b and is native since wave 1: `connection`, every preference presenter, `commands.reconnect`, the FX pricing/blotter/execution presenters and their machines, the four credit presenters and the four RFQ machines — `rfqCountdown` joined `MachineFactories` in slice 3, the seam's first new member — and slice 4's eight: the equities presenters (`watchlist`, `candleSeries`, `depth`, `ordersBlotter`, `positions`), the two equities workspace singletons (`eqWorkspace`, `eqDrawings`) and the machine `orderTicket` — and slice 5's nine admin members (`throughput`, `throughputMetric`, `latencyMetric`, `errorRateMetric`, `topology`, `eventLog`, `sessions`, `sessionsKpi`, the singleton `incident`) — and slice 6's five shell members (`auth`, `bootGate`, `workspaceNav`, `animationDirector`, the machine `boot`) — and slice 7 wave 1's twelve workspace members (`layoutFor`/`machines.layout`, `dockLayoutStore`, the dock bridges, `resetWorkspaceLayout`, `dockedPanelIdsFor`, `workspaceLayoutResets$`, `layoutPresets`, `commands.reportDetachedPanels`, `jarvisPanels`), whose synchronous rules are SHARED rxjs-free controllers in `@rtc/client-core` (`createWorkspaceDock` and kin) — and slice 7 wave 2's Jarvis family (`jarvis`, `jarvisUsage`, `jarvisDriver`, `jarvisDemo`, the internal narrator), likewise over shared controllers (`createJarvisController`, `applyDriveCommand`, `createDemoStepWatch`, the narrator gate); each sibling builds the family and its workspace itself and the base app stands down through `CoreSeams.nativeJarvis` (no availability request, history source or narrator of its own; one `workspaceLayout` writer); `createApp`'s `CoreSeams` argument additionally points the base app's remaining INTERNAL readers — its `AnimationDirector` and workspace seed — at the native members, so a fill or an FX execution lands where the UI looks and no DATA port a native member holds is held twice (the base's own preference presenters still read the preferences port beside the native ones — port-backed and idempotent, so nothing is stale); strangler scaffolding, deleted in slice 8) and delegate the rest to `@rtc/client-core`; a committed `parity.json` per core plus a reference-inequality drift test keep that provenance honest (a member marked `"native"` must not literally be the RxJS instance, a `"delegated"` one must). See [ADR-006](docs/adr/ADR-006-pluggable-application-core.md) and `docs/architecture/22-pluggable-application-core.md` (§22).
