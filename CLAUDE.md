...
packages/
  ...
  client-core-async/   @rtc/client-core-async   — Sibling application core on async/await + AsyncIterable: a Store/Topic/spawn/sleep kernel, with `bridge/` the only place rxjs is imported as a value. Slice 1b: seventeen members native (connection, all fifteen preference presenters, commands.reconnect), the rest delegate to @rtc/client-core; a committed parity.json + reference-inequality drift test track native-vs-delegated provenance. Depends on core-api, client-core (strangler-phase only), domain (+ rxjs, @rx-state/core as types outside bridge/).
  client-core-effect/  @rtc/client-core-effect  — Sibling application core on Effect-TS (`effect` ^3.22.2): ManagedRuntime-owning composition root, Stream/SubscriptionRef bridge. Slice 1b: seventeen members native, the rest delegate to @rtc/client-core. Same strangler shape and parity manifest as client-core-async. Depends on core-api, client-core (strangler-phase only), domain, effect (+ rxjs as types outside bridge/).
