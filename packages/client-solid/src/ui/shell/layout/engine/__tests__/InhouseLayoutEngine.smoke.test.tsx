...
describe("InhouseLayoutEngine", () => {
  ...
  it("confines a panel that throws during render to a scoped panel-error fallback, leaving sibling panels intact (no app-wide white screen)", () => {
    ...
    // plain function. The crash this guards against happens when the render
    // itself later evaluates that returned element (e.g. ChartPanel's own
    // body throwing), not when the registry function is called. So the fake
    // here must throw from within an actual rendered component, matching
    // that real shape — a registry entry that throws directly, as a plain
    // function call, would throw one call frame too early to exercise the
    // boundary at all.
  ...
  describe("live state updates (Solid-port regression: components must not freeze at initial render)", () => {
    // These have no React analogue — React re-invokes the whole component
    // function on every prop change, so a top-level `const` derived from
    // `state` naturally recomputes. Solid component bodies run ONCE; a
    // top-level `const` would freeze the very first render forever. Driving
    // `state` through a REAL signal (matching how the real `useLayout(tab)`
    // ViewModel member feeds InhouseLayoutEngine in App.tsx) is exactly the
    // scenario a plain-function/frozen-const test double would hide — see
    ...
    it("collapsing a panel via a live state update strips it and hides its body, without remounting its sibling", () => {
      ...
      const ratesBodyBefore = screen.getByTestId("rates-body");
      ...
      expect(screen.getByTestId("rates-body")).toBe(ratesBodyBefore);
    ...
    it("maximizing a panel via a live state update recomputes boundaryPath/strippedByMaximize and strips the sibling", () => {
      ...
      expect(
        screen.getByTestId("panel-fx-rates").getAttribute("data-maximized"),
      ).toBe("true");
      expect(
        screen.getByTestId("panel-fx-analytics").getAttribute("data-strip"),
      ).toBe("true");
      ...
      setLayoutState({ ...state, maximized: null });
      expect(
        screen.getByTestId("panel-fx-analytics").getAttribute("data-strip"),
      ).toBe("false");
      expect(screen.getByTestId("analytics-body")).toBeTruthy();
    ...
    it("a resize drag driven off a live state signal keeps updating --split-size on the SAME cell element across ticks", () => {
        ...
        return (
          <InhouseLayoutEngine
            ...
            onResize={(
              ...
              // Mirrors resizeAt's `{...node, sizes, initialPx: undefined}`
              // structural-sharing shape at path=[] (see LayoutMachine.ts):
              // a fresh root object, same child object references.
              setLayoutState((prev) => {
                return { ...prev, root: { ...prev.root, sizes } };
              });
            }}
          />
        );
      ...
      const cellBefore = screen
        .getByTestId("panel-fx-rates")
        .closest("[data-testid^='cell-']");
      const handle = screen.getByTestId("handle--0");
      fireEvent.pointerDown(handle, { pointerId: 5, clientX: 60, clientY: 0 });
      fireEvent.pointerMove(handle, { clientX: 90, clientY: 0 });
      fireEvent.pointerUp(handle, { pointerId: 5, clientX: 90, clientY: 0 });
      ...
      const cellAfter = screen
        .getByTestId("panel-fx-rates")
        .closest("[data-testid^='cell-']");
      expect(cellAfter).toBe(cellBefore);
      expect(
        (cellAfter as HTMLElement).style.getPropertyValue("--split-size"),
      ).not.toBe("0.6");
    });
  });
});
