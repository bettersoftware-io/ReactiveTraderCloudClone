/* The desk-panel interpreter's pure half (pluggable-core slice 7): frames,
 * transforms and renderers, plus one pure step per source. Shared by every
 * application core — the RxJS `composePanelStream` and the sibling cores'
 * native panel streams subscribe their own way and fold these. rxjs-free by
 * rule. */
...
export function toneFromTradeStatus(status: TradeStatus): PanelTone {
  switch (status) {
...
export function unknownSourceFrame(_source: never): Frame {
  return { kind: "empty" };
}
...
export function applyTransform(frame: Frame, transform: PanelTransform): Frame {
  if (frame.kind === "empty") {
    return frame;
  }
...
/** One fx tick folded onto a symbol's accumulated series (the `fxTicks`
 * source's per-tick step), capped at `MAX_POINTS_PER_SERIES`. */
export function appendTickPoint(
  points: readonly PanelPoint[],
  tick: PriceTick,
): readonly PanelPoint[] {
  return capPoints([...points, { t: tick.creationTimestamp, v: tick.mid }]);
}

/** A `priceHistory` source's snapshot for one symbol, as a named series. */
...
/** The series frame both fx sources emit once every symbol has a series. */
export function seriesFrame(series: readonly NamedSeries[]): Frame {
  return { kind: "series", series };
}

/** The `analytics` source's table frame for one positions update. */
export function analyticsTableFrame(updates: PositionUpdates): Frame {
  return {
    kind: "table",
...
/** The `blotter` source's table frame: the most recent trades, newest first. */
export function blotterTableFrame(trades: readonly Trade[]): Frame {
  const recent = [...trades]
    .sort((a, b) => {
...
/** A source frame through `spec`'s transforms, in order, then its viz — the
 * whole per-emission interpretation of one desk panel. */
export function renderPanelFrame(spec: PanelSpecV1, frame: Frame): PanelData {
  return renderViz(
    spec.transforms.reduce(applyTransform, frame),
    spec.viz,
    spec.annotations ?? [],
  );
}
