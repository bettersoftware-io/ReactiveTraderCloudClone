...
import { DEFAULT_FORCE_BOOT_ANIMATION } from "@rtc/domain";

import { collect } from "#/harness/collect";
import type { MakeHarness } from "#/harness/harness";
import { settle } from "#/harness/settle";

export function describeForceBootAnimationContract(
  label: string,
  ...
  describe(label, () => {
    it("enabled$ replays the default synchronously and follows set", async () => {
      ...
      try {
        const p = h.app.presenters.forceBootAnimation;
        const c = collect(p.enabled$);
        expect(c.values).toEqual([DEFAULT_FORCE_BOOT_ANIMATION]);
        p.set(false);
        await settle();
        expect(c.values).toEqual([DEFAULT_FORCE_BOOT_ANIMATION, false]);
        c.unsubscribe();
      } finally {
        await h.teardown();
      }
