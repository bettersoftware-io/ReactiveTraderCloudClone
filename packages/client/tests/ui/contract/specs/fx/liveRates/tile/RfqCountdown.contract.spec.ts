...
describe("RfqCountdown", () => {
  it("shows the rounded-up seconds remaining", () => {
    const cd = mount(RfqCountdown, { props: { remainingMs: 7_200, totalMs: 10_000 } });
    expect(cd.caption()).toBe("8s remaining");
  });

  it("fills the progress bar proportionally", () => {
    const cd = mount(RfqCountdown, { props: { remainingMs: 5_000, totalMs: 10_000 } });
  ...
  it("switches to the warning colour below 30% remaining", () => {
    const cd = mount(RfqCountdown, { props: { remainingMs: 2_000, totalMs: 10_000 } });
  ...
  it("shrinks the bar as the remaining time ticks down", () => {
    const cd = mount(RfqCountdown, { props: { remainingMs: 8_000, totalMs: 10_000 } });
