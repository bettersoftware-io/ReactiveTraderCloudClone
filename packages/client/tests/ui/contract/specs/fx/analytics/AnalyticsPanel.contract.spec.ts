...
const historic = (usdPnl: number, timestamp = "2026-06-13T00:00:00Z"): HistoricPosition => ({
  timestamp,
  usdPnl,
});
...
describe("AnalyticsPanel", () => {
  ...
  it("summarises the latest historic P&L figure", () => {
    const panel = mount(AnalyticsPanel, {
      hooks: { useAnalytics: updates({ history: [historic(0), historic(1_500_000)] }) },
    });
  ...
  it("transitions from loading to loaded when analytics begin streaming", () => {
    const panel = mount(AnalyticsPanel, { hooks: { useAnalytics: null } });
    expect(panel.isLoaded()).toBe(false);
    panel.emit({ useAnalytics: updates({ history: [historic(0), historic(-2_500)] }) });
    expect(panel.isLoaded()).toBe(true);
  ...
  it("updates the summarised P&L figure when newer analytics stream in", () => {
    const panel = mount(AnalyticsPanel, {
      hooks: { useAnalytics: updates({ history: [historic(0), historic(500)] }) },
    });
    expect(panel.latestPnlText()).toBe("+500");
    panel.emit({ useAnalytics: updates({ history: [historic(0), historic(500), historic(12_500)] }) });
  ...
  it("shows the stale overlay after a disconnect/reconnect with no fresh data", () => {
    const data = updates();
    const panel = mount(AnalyticsPanel, {
      hooks: { useAnalytics: data, useConnectionStatus: ConnectionStatus.CONNECTED },
    });
    ...
    panel.emit({ useAnalytics: updates({ history: [historic(0), historic(7_000)] }) });
    expect(panel.isStale()).toBe(false);
