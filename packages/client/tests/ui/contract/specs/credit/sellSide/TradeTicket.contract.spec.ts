...
import { Direction, RfqState, type Rfq, type Quote, type QuoteState, type Instrument } from "@rtc/domain";
import { mount } from "@ui-contract/mount";
import { TradeTicket } from "@ui-contract/components";

const instrument: Instrument = {
  id: 1, name: "US Treasury 10Y", cusip: "912828ZQ6", ticker: "T 1.5 02/34",
  maturity: "2034-02-15", interestRate: 1.5, benchmark: "10Y",
};
...
const quote = (state: QuoteState): Quote => ({ id: 100, rfqId: 10, dealerId: 1, state });
...
describe("TradeTicket", () => {
  it("shows the instrument details and an active price form when awaiting a response", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq(), quote: quote({ type: "pendingWithoutPrice" }), instrument },
    });
  ...
  it("falls back to an instrument-id label when the instrument is unknown", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq(), quote: quote({ type: "pendingWithoutPrice" }), instrument: undefined },
    });
  ...
  it("shows the quoted price for a quote that already has a price", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq(), quote: quote({ type: "pendingWithPrice", price: 99 }), instrument },
    });
    expect(ticket.isActive()).toBe(false);
  ...
  it("shows the RFQ Cancelled message when the RFQ was cancelled but the quote stayed pending", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Cancelled }), quote: quote({ type: "pendingWithoutPrice" }), instrument },
    });
  ...
  it("shows Expired in the inactive branch when an expired RFQ still has a pending quote", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Expired }), quote: quote({ type: "pendingWithoutPrice" }), instrument },
    });
    expect(ticket.isActive()).toBe(false);
  ...
  it("shows Closed in the inactive branch when a closed RFQ still has a pending quote", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Closed }), quote: quote({ type: "pendingWithoutPrice" }), instrument },
    });
    expect(ticket.isActive()).toBe(false);
  ...
  it("shows the RFQ Expired message when the RFQ expired with a priced quote", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Expired }), quote: quote({ type: "rejectedWithPrice", price: 90 }), instrument },
    });
  ...
  it("shows a generic Responded message for a closed RFQ with a rejected-without-price quote", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Closed }), quote: quote({ type: "rejectedWithoutPrice" }), instrument },
    });
  ...
  it("shows the RFQ Cancelled responded message for a cancelled RFQ with a priced quote", () => {
    const ticket = mount(TradeTicket, {
      props: { rfq: rfq({ state: RfqState.Cancelled }), quote: quote({ type: "rejectedWithPrice", price: 80 }), instrument },
    });
