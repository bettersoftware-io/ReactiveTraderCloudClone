...
import { Direction, RfqState, ADAPTIVE_BANK_NAME, type Rfq, type Quote, type Instrument, type Dealer } from "@rtc/domain";
...
describe("SellSidePanel", () => {
  it("shows the panel title", () => {
    const panel = mount(SellSidePanel, { hooks: { useInstruments: instruments, useDealers: withAdaptive } });
  ...
  it("shows the empty state when Adaptive Bank is not among the dealers", () => {
    const panel = mount(SellSidePanel, {
      hooks: { useInstruments: instruments, useDealers: [{ id: 2, name: "Citi" }], useRfqs: [rfq(1)] },
    });
  ...
  it("shows the empty state when there are no RFQs", () => {
    const panel = mount(SellSidePanel, {
      hooks: { useInstruments: instruments, useDealers: withAdaptive, useRfqs: [] },
    });
    expect(panel.emptyMessage()).toMatch(/no rfqs for adaptive bank/i);
  });
  ...
  it("renders a ticket only for RFQs that carry an Adaptive Bank quote", () => {
    const panel = mount(SellSidePanel, {
      hooks: { useInstruments: instruments, useDealers: withAdaptive, useRfqs: [rfq(1), rfq(2)] },
      parametric: {
        // RFQ 1 has an Adaptive Bank quote; RFQ 2 has only a Citi quote → skipped.
        quotesForRfq: {
          1: [adaptiveQuote(1)],
          2: [{ id: 99, rfqId: 2, dealerId: 2, state: { type: "pendingWithoutPrice" } }],
        },
      },
    });
  ...
  it("renders a ticket once its Adaptive Bank quote streams in", () => {
    const panel = mount(SellSidePanel, {
      hooks: { useInstruments: instruments, useDealers: withAdaptive, useRfqs: [rfq(1)] },
    });
