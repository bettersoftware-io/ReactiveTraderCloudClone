...
pixel-parity; a future `:solid` runner is discovered by `tests/visual-diff/run-all.ts`
automatically.
...
changed; `pnpm test --force` re-runs for real. `pnpm test:visual-diff` is never
