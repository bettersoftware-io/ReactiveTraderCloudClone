/**
 * The saved-layouts controller (Phase 6b; rxjs-free since pluggable-core
 * slice 7 — every application core wraps it, the RxJS one in
 * `createLayoutPresets.ts`) — every rule of the View menu's
 ...
 * channel, the per-tab snapshot sources, and the id counter. The list itself
...
import { createLayoutReducer, layoutStaticIds } from "#/layout/layoutReducer";
...
/** Where the controller publishes each tab's summary list — the one
 * stream-shaped part of saved layouts, so each application core supplies its
 * own (the RxJS core a `BehaviorSubject` per tab: `createLayoutPresets`). */
export interface PresetSummaryChannel {
  /** `tab`'s replay-current summary stream; `initial` is read only when the
   * tab has no summaries yet. */
  streamFor(
    tab: WorkspaceTab,
    initial: () => readonly LayoutPresetSummary[],
  ): Stream<readonly LayoutPresetSummary[]>;
  /** Replace `tab`'s summaries (creating the tab's stream if need be). */
  publish(tab: WorkspaceTab, summaries: readonly LayoutPresetSummary[]): void;
}

export function createLayoutPresetsController(
  deps: LayoutPresetsDeps,
  summaries: PresetSummaryChannel,
): LayoutPresetsPresenter {
  ...
  function presetsFor(
    ...
    return summaries.streamFor(tab, () => {
      return summarizeLayoutPresets(parsedListFor(tab));
    });
  }
  ...
  function publishStoredList(tab: WorkspaceTab): void {
    summaries.publish(tab, summarizeLayoutPresets(parsedListFor(tab)));
  }
 ...
 *
 * Folds the removals through the layout REDUCER (`createLayoutReducer`)
 * rather than re-deriving the tree surgery: `removePanel` already collapses
 * an emptied dock column back to exactly the pre-dock tree and drops
 * `maximized`/`collapsed` entries that named the removed id. The LIVE machine
 * for the tab is never touched — a save must not alter what the user is
 * looking at.
 */
function withoutDockedLeaves(
  ...
  return dockedPanelIds.reduce((current, panelId) => {
    return reduce(current, { type: "removePanel", id: panelId });
  }, state);
}
