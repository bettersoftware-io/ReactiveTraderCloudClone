...
describe("useDraggableDialog", () => {
  it("starts centered with a zero offset", () => {
    ...
    expect(handle.state.offset).toEqual({ x: 0, y: 0 });
  });
  ...
  it("updates the offset on a pointer drag, clamped to the viewport", () => {
    ...
    handle.commit(() => {
      handle.state.headerProps.onPointerMove(
        pointerEvent({ clientX: 140, clientY: 130 }),
      );
    });

    expect(handle.state.offset).toEqual({ x: 40, y: 30 });
    expect(handle.state.dialogStyle.transform).toBe("translate(40px, 30px)");
  });
  ...
  it("clamps the offset so the dialog never leaves the viewport", () => {
    ...
    expect(handle.state.offset.x).toBe(276);
    expect(handle.state.offset.y).toBe(226);
  });
  ...
  it("resets the offset to zero when the dialog closes", () => {
    ...
    expect(handle.state.offset).toEqual({ x: 40, y: 20 });

    handle.rerender({ open: false });
