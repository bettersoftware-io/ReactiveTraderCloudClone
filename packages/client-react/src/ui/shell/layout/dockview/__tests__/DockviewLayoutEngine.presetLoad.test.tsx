...
// (mirrors the sibling DockviewLayoutEngine.docked.test.tsx, whose comment
// explains why `ResizeObserverCtor` is declared below the cases). The stub must
// be in place before the FIRST `createDockEngine` call, which here includes the
// blob-capturing factories the cases call, not only the mounted bridge.
beforeAll(() => {
 ...
 * a preset-LOAD path, before any of the phase's code depends on it. A preset
 * load is not a workspace reset: the reset clears the tab's blob and expects
 * the seed back, while a load writes a DIFFERENT saved blob and expects THAT
 * back, with layer-2 state replaced in the same commit. #738 found layer-2
 * replay effects reaching only a SECOND engine instance under specific
 * ordering, so none of that is assumed here.
 *
 * Every case performs the same one-commit act Task 5's `load` will perform —
 * `store.save(tab, preset)`, then the layer-2 props and the reset counter
 * changed together in a single `rerender` — and then asserts a POSITIVE,
 * visible consequence of the loaded blob (a group count, a stacked pair of
 * tabs in one group element, a docked panel's body), never merely "not the old
 * one".
 *
 * The blob being loaded is CAPTURED FROM A REAL ENGINE, never hand-written:
 * see `createEngineWrittenStackedBlob` for how, and for why that matters. */
describe("DockviewLayoutEngine preset load (layoutResets rebuild)", () => {
  // The premise every case below rests on. Without it, "the DOM reflects the
  // loaded blob" could pass while the two blobs were structurally identical
  // (a driver blob the engine silently scrubbed back to the seed would do
  // exactly that), and all four answers would be vacuous.
  ...
  // QUESTION 2 — does layer 2 replay onto the NEW engine, or is it lost to
  // the dispose? The collapse arrives in the SAME commit as the rebuild, so
  // that commit's own collapse effect still closes over the OLD (by then
  // disposed) engine and is refused by the stale-closure guard; only the
  // follow-up commit `setLiveEngine(fresh)` schedules can apply it.
  ...
  it("re-adds a docked panel the loaded blob does not contain", () => {
    const recorded = createRecordingStore(createEngineWrittenSeedBlob());
    const preset = createEngineWrittenStackedBlob();
    ...
    page.mount({ registry, store: recorded.store, docked: ["panel-dyn-1"] });
    page.pressDockContainer();
 ...
 * has not yet forgotten), oldest first. */
