...
// (see DockviewLayoutEngine.strictMode.test.tsx / .floating.test.tsx).
beforeAll(() => {
...
/** Phase 6b Task 7 — the `onSnapshotSourceChange` slot: hands the controller
 * a `() => string` that reads the LIVE engine's blob on demand, right after
 * construction, and `null` right before that engine is disposed. A preset
 * SAVE reads through whatever source is currently registered, so this is
 * what makes a save capture the dock exactly as it looks right now, rather
 * than whatever the last debounced `onLayoutChange` happened to persist. */
describe("dockview bridge layout-snapshot registration", () => {
  it("hands the controller a live source on mount, matching the store's next write after a collapse", async () => {
  ...
  it("re-registers on a workspace-reset rebuild: null, then a NEW source reflecting the REBUILT engine", () => {
    const recorded = createRecordingStore();
 ...
 * sibling docked spec's identical NEW-1 witness. A disposed engine's
 * panel-less serialize produces a grid whose leaves carry `"views":[]`, not
 * a grid missing leaves entirely, and that shape ALSO fails `sharesGroup` —
 * so `sharesGroup(...) === false` alone cannot tell "genuinely rebuilt,
