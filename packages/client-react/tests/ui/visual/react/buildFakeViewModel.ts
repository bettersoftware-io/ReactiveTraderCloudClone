...
export function buildFakeViewModel(data: AppData): ViewModel {
  return {
    ...
    useLayout: (tab: WorkspaceTab) => {
      return {
        state: seededLayoutStateFor(data, tab),
        maximize: noop,
        restore: noop,
        collapse: noop,
        expand: noop,
    ...
    // would churn on the domain default flip.
    // Since #752 every App scenario's fixture names its engine
    // explicitly (15 inhouse, 11 dockview), so this fallback is now a
    // BACKSTOP for a scenario that forgets rather than the thing holding
    // the matrix in place.
    useLayoutEngine: () => {
      return {
        engine: data.layoutEngine ?? "inhouse",
        setEngine: noop,
      };
    },
    useDockLayoutStore: () => {
