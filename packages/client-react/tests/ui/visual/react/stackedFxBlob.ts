 ...
 *
 * Spelled as a literal + `JSON.stringify` rather than a minified string so
 * the shape is readable and diffable. `JSON.stringify` emits the captured
 * blob byte for byte (key order is insertion order), so what reaches the
 * store is unchanged — this is a legibility change, not a data one. The
...
export const STACKED_FX_BLOB = JSON.stringify(STACKED_FX_LAYOUT);
