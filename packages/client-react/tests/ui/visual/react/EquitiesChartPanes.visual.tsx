 ...
 * visual tier). The candle series/constants are duplicated (not imported)
 * from `EquitiesChartInteractive.visual.tsx` for the same reason that file
 * gives: each visual wrapper stays self-contained per client.
 */
...
function ForcedPaneChart({
  ...
  const vm = chartVm(CANDLES, LIVE_RATE, false, { viewport, kind: "candles" });
  const cross = cursor
