 ...
 * timers, no Date.now. Ported verbatim from web's
 * `client-react/src/ui/equities/blotter/useNewestOrderId.ts` (client-react-native
 * may not depend on client-react — see the repo's dependency rule) with a ref
 * only ever read/written inside an effect, never during render. The very
 * first render just captures the initial id set (nothing counts as "new" on
 * mount); from then on, whichever order's id wasn't in the previous set
 * becomes the flagged row. There is no separate "clear" step: when a newer
 * order later appears the flag simply moves there.
 */
